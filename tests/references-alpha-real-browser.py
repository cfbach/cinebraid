...
def verify_reference_dialog_navigation(page, entity_id):
    ...
    for width in (1280,1440,1920):
        ...
        # EV2-7: Production Media is a Build coverage method. Cancel in the picker returns to Build coverage, focused on that method.
        build=page.locator('[data-rd-action="build"]').first;build.focus();build.click()
        method=page.locator('#modal [data-bc-method="media"]');method.wait_for();method.focus();method.click()
        page.wait_for_selector('#rd-picker-cancel');page.locator('#rd-picker-cancel').click()
        page.wait_for_function("() => !document.getElementById('modal').classList.contains('hidden') && !!document.querySelector('#modal [data-build-coverage][data-bc-step=method]') && document.activeElement?.matches('#modal [data-bc-method=media]')")
        evidence.append({'width':width,'dismissal':'Cancel','returnedTo':'Build coverage method','focusRestored':True})
        page.locator('#modal [data-build-coverage] [data-bc-action="close"]').first.click()
