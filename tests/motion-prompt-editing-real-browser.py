...
LABEL='Motion prompt editing real-browser audit'
...
port=free_port(); workspace=disposable_workspace('motion-prompt-editing', sample=False); server=subprocess.Popen(['node','server.js'],cwd=ROOT,env=workspace.env(port),stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
try:
    wait(port)
