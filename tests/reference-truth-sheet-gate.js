...
/* THE HAND-DROPPED PATH, REPORTED HONESTLY RATHER THAN GUESSED.
   This is the slice's STOP/HOLD question and the answer is a limitation, so it is
   pinned as one: an import declares nothing, therefore a hand-dropped SHEET is
   `undeclared` and the gate does not stop it. Closing that needs a declaration at
   import time — a new contract — and this test exists so that the day someone
   adds one, this expectation fails and has to be updated deliberately. */
function testHandDroppedImportDeclaresNothing() {
  const source = fs.readFileSync(path.join(ROOT, "public/library-tools.js"), "utf8");
  const intake = source.slice(source.indexOf("window.doIntake"), source.indexOf("window.doIntake") + 2400);
  ok(/original\.push\(\{ stored: d\.name, original: f\.name/.test(intake),
    "the import writer still records only stored/original plus an optional state target");
  for (const field of ["coverageJobType", "coverageSheetType", "coverageCrop"]) {
    ok(!intake.includes(field), `the import writer declares no ${field} — this is the missing seam`);
  }
  eq(Coverage.referenceArtifactStructure({ stored: "CHAR-A-CANDIDATE-M9X-01.png", original: "TURNAROUND.png" }),
    "undeclared", "a hand-dropped sheet is UNDECLARED — a known, reported gap, not a silent pass");
  eq(Coverage.artifactMayHoldPrimaryAuthority("undeclared"), true,
    "undeclared stays eligible on purpose: refusing it would make every hand-dropped primary impossible");
}
...
function testTheBoundaryRefusesASheet() {
  ...
  eq(plain.wrote, true, "an ordinary hand-dropped identity image may still become the identity");

  /* 6 — FILENAME FALSE POSITIVE. The entity id carries the old token. */
  const bedsheet = approveThrough(kernel, manual,
    entityProject({}, "props", "PROP-BEDSHEET"), "props", "PROP-BEDSHEET");
  eq(bedsheet.wrote, true, "a prop called Bedsheet must still be able to have an identity reference");
}
...
function testUnresolvableClassifierFailsClosed() {
  ...
  const manual = installTestManualActionSource(kernel);
  const result = approveThrough(kernel, manual, entityProject({}));
  eq(result.wrote, false, "with no classifier the kernel must not grant identity authority");
...
async function main() {
  ...
  testHandDroppedImportDeclaresNothing();
