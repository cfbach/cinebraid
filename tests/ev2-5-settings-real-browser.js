...
let server,browser,page,base;const checks=[],errors=[],requests=[],blocked=[];let saveFails=false,grantsFail=false,assistantCalls=0,comfyCalls=0,accountFixture=false,accountRemoved=false,holdSave=false,heldSave=null;
...
(async()=>{try{
 ...
 for(const [width,height] of [[1280,720],[1440,900],[1920,1080],[390,844]]){
  ...
  if(width===390){await page.getByRole('combobox',{name:'Settings section',exact:true}).selectOption('connections');}else{await page.locator('.studio-nav-links a[href="#/settings/connections"]').click();}
  await page.locator('[data-settings-tab="connections"]').waitFor();check('Connections reachable '+width,await page.locator('.studio-connection').count()===7);await screenshot('connections-'+width);
  for(const section of ['generation','integrations','fal','assistant','assistant-custom','files','access','recovery','project','project-recovery','setup']){
   await go(section);check(section+' fits '+width,await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));check(section+' fields named '+width,await page.locator('.settings-selected-tab').evaluate(panel=>[...panel.querySelectorAll('input,select,textarea')].every(el=>el.type==='hidden'||el.getAttribute('aria-label')||el.labels?.length)));
 ...
 await go('connections');await go('generation');check('Draft survives section navigation',await page.locator('#cfg-fal-text-model').inputValue()==='synthetic/long-model-name-kept-as-an-unsaved-operation-default');check('Draft focus restored',await page.locator('#cfg-fal-text-model').evaluate(el=>el===document.activeElement));
 ...
 const historyBefore=fs.readFileSync(fixture,'utf8');await page.getByRole('button',{name:'Disconnect',exact:true}).click();await page.getByRole('button',{name:'DISCONNECT',exact:true}).click();await page.waitForTimeout(200);check('Disconnect preserves local project history',historyBefore===fs.readFileSync(fixture,'utf8'));check('Disconnected fixture account removed from list',await page.locator('[data-account-status="expired"]').count()===0);
 ...
 fs.writeFileSync(path.join(OUT,'browser-validation.json'),JSON.stringify({passed:true,checks,browser:browser.version(),isolation:{disposable:true,port,serverOutbound:'blocked',providers:'mocked checks only',realCredentials:false,realProjects:false},requests,errors,blocked},null,2));console.log(JSON.stringify({passed:true,checks:checks.length,captures:OUT}));
