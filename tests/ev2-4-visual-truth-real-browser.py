...
try:
 ...
 with require_browser('EV2-4 visual truth correction')() as pw:
  ...
  page.locator('a[href="#/shots"]').last.click();page.locator('.wb-return [data-wb="back"]').wait_for()
  check('Unsaved presentation follows draft outside Bible',page.locator('#save-state').inner_text().strip()=='Unsaved Bible edit')
  page.locator('.wb-return [data-wb="back"]').click();page.locator('#wb-text').wait_for();page.locator('[data-wb="discard"]').click()
