...
function project() {
  return {
    shots: [{
      id: "SH-1",
      keyframes: [{ id: "FR-A", label: "A", winner: "A.png" }, { id: "FR-C", label: "C", winner: "C.png" }],
      candidateFiles: [{ stored: "A.png", frameId: "FR-A" }, { stored: "C.png", frameId: "FR-C" }],
      creationBrief: { frameWorkflows: { "FR-A": { characterStateSelections: { [KAI]: "state-rain" } } } },
    }],
    characters: [{
...
control("NC-6a", "capturing the binding after the paid request instead of before the commit", async () => {
  ...
  /* RECEIPT 1 — the anchor matched both compiled branches, not one. */
  assert.strictEqual((read("fal-generation.js").match(/applyBindingRecord\(job, bindingRecordFor/g) || []).length, 2,
    "the shipped dispatcher must apply the record on both compiled branches");
  assert.strictEqual((source.match(/applyBindingRecord\(job, bindingRecordFor/g) || []).length, 0,
    "the control must actually remove the pre-commit capture");
...
async function main() {
  ...
  try {
    ...
    console.log(
      ...
      + "the paid POST, a reader that re-resolves history from current project state, and a default continuity state "
      + "minted for an entity that declares none — every one detected by the "
