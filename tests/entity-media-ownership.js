...
eq(Ownership.mediaOwnerId(unclaimedIndex, UNCLAIMED), CHILD,
  "a hand-imported file no record claims resolves by the LONGEST matching prefix — `CHAR-SWEEP-YOUNG` is a more specific statement about those bytes than `CHAR-SWEEP`");
ok(!Ownership.entityOwnsMedia(unclaimedIndex, ADULT, UNCLAIMED), "so it does not also land in the parent's pool");
ok(Ownership.filterEntityMedia(unclaimedIndex, CHILD, allFiles({ unclaimed: true })).some((row) => row.name === UNCLAIMED),
  "and it IS visible to its owner — a creator who drops a reference into anchors/ must still see it");

const noClaims = Ownership.buildEntityOwnerIndex(project({ claims: false }), "characters");
eq(Ownership.filterEntityMedia(noClaims, ADULT, allFiles()).map((row) => row.name), ADULT_FILES,
  "a project with no candidate rows at all — every project before the first pass — still resolves correctly by prefix specificity alone");
eq(Ownership.filterEntityMedia(noClaims, CHILD, allFiles()).map((row) => row.name), CHILD_FILES, "for the child too");
...
eq(contested[0].attributedTo, CHILD,
  "and it is attributed by the fallback rule, which resolves it to the child — the leak is closed even on a contaminated project, without editing a single stored row");
...
ok(/filterEntityMedia\(entityOwnerIndex\(list\), it\?\.id, rows\)/.test(entities),
  "entityMedia() — the reader every entity surface goes through — uses the exact resolver");
