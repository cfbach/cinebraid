...
const { createContinuityCache } = require("../continuity/continuity-cache");
const ContinuityJson = require("../continuity/continuity-json");
...
const AgentSuite = require("../assistant/agent-suite");
const { registerFalGeneration } = require("../generation/fal/fal-generation");
const { registerComfyGeneration } = require("../generation/comfyui/comfy-generation");
const { registerCivitaiGeneration } = require("../generation/civitai/civitai-generation");
const { registerAutomationRuns } = require("../automation/automation-runs");
const { createGenerationPoller } = require("../generation/generation-poller");
const { isAccountCallbackPath, registerAccountConnections } = require("../accounts/accounts-api");
...
function resolveWorkspacePath(value, fallback = "") {
  ...
  const expanded = raw.startsWith("~/") || raw === "~"
    ? path.join(process.env.HOME || process.env.USERPROFILE || path.resolve(__dirname, "../.."), raw.slice(1))
    : raw;
...
(function migrate() {
  const oldData = path.join(path.resolve(__dirname, "../.."), "data", "project.json");
  const oldProj = path.join(path.resolve(__dirname, "../.."), "project");
...
const PROJECT_BUILDER_DIR = path.join(path.resolve(__dirname, "../.."), "resources", "project-builder");
const { projectBuilderContract } = require("../project/project-builder-contract");
...
const ReferenceReview = require("../authority/reference-review-contract");
const {
