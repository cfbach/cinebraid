# CineBraid 6.6.6 Private Test 1 — install
...
| File | For |
|---|---|
| `cinebraid-6.6.6-private.1-windows.zip` | Windows |
| `cinebraid-6.6.6-private.1-runtime.tar.gz` | Linux, macOS, DGX Spark |
| `SHA256SUMS.txt` | checksums for every asset |
| `cinebraid-6.6.6-private.1-manifest.json` | machine-readable release manifest |
| `CINEBRAID_v6.6.6-private.1_RELEASE_NOTES.md` | what changed, and the known limitations |
...
```powershell
Get-FileHash .\cinebraid-6.6.6-private.1-windows.zip -Algorithm SHA256
```
...
Both archives extract to a single top-level folder, `cinebraid-6.6.6-private.1/`.
...
```powershell
Expand-Archive -Path .\cinebraid-6.6.6-private.1-windows.zip -DestinationPath "$env:USERPROFILE\CineBraid"
```

Linux / macOS:

```bash
tar -xzf cinebraid-6.6.6-private.1-runtime.tar.gz -C ~/
```

A path containing spaces is supported. Every startup script resolves its own
directory and quotes it, so `%USERPROFILE%\My CineBraid Builds\` works. When you
...
Open `http://127.0.0.1:4477`. The window title reads **CineBraid 6.6.6 Private
Test 1**. The shipped `cinebraid-sample` project opens in manual-first mode.
Opening it does not write to it.

## 6. Optional — point the assistant at a local OpenAI-compatible server

New in this release. In **Settings → Assistant**, choose **Custom server** and set:

| Field | Value |
|---|---|
| Custom base URL | your server's OpenAI-compatible base, including `/v1` |
| Custom text model | the model name the server actually serves |
| Sampling temperature | optional; blank sends nothing |
| Top-K sampling | optional; blank sends nothing |
| Model thinking | *Ask the server to skip thinking* for a reasoning server |

Then press **Test connection**. A reasoning server that returns no final answer
is the case *Ask the server to skip thinking* exists for.

Set **Vision assistant** explicitly. Leaving it on *Same as main assistant* points
