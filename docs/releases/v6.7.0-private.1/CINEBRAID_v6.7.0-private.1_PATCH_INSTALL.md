# CineBraid 6.7.0 Private Test 1 — install
...
| File | For |
|---|---|
| `cinebraid-6.7.0-private.1-windows.zip` | Windows |
| `cinebraid-6.7.0-private.1-runtime.tar.gz` | Linux, macOS, DGX Spark |
| `SHA256SUMS.txt` | checksums for every asset |
| `cinebraid-6.7.0-private.1-manifest.json` | machine-readable release manifest |
| `CINEBRAID_v6.7.0-private.1_RELEASE_NOTES.md` | what changed, and the known limitations |
...
```powershell
Get-FileHash .\cinebraid-6.7.0-private.1-windows.zip -Algorithm SHA256
```
...
Both archives extract to a single top-level folder, `cinebraid-6.7.0-private.1/`.
...
```powershell
Expand-Archive -Path .\cinebraid-6.7.0-private.1-windows.zip -DestinationPath "$env:USERPROFILE\CineBraid"
```

Linux / macOS:

```bash
tar -xzf cinebraid-6.7.0-private.1-runtime.tar.gz -C ~/
```
...
Open `http://127.0.0.1:4477`. The window title reads **CineBraid 6.7.0 Private
Test 1**. The shipped `cinebraid-sample` project opens in manual-first mode.
Opening it does not write to it.

## 6. Optional — validate an OFP draft document

New in this release, and the only way to reach the OFP contract from outside the
test suites:

```bash
node scripts/validate-ofp.js <path-to-document.ofp.json>
```

It reports what it finds and changes nothing — not the document, and not any
project. There is no OFP surface in the application UI in this build, and no
project is stored in the OFP format. `format.version` in a document written
against this contract is `1.0-draft.1`; it is not the application version and
never follows it.
