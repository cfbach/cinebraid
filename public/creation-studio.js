...
function guidedMotionPanel(s, current, takes, open = false) {
  ...
  const label = approved ? "Approved video ready" : videos.length ? `${videos.length} video${videos.length === 1 ? "" : "s"} to review` : approvedFrames.length > 1 ? `Animate between ${approvedFrames.length} approved frames` : needsApprovedStill ? "Animate the approved first frame" : "Generate video from the written direction";
