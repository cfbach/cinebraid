...
function assetStatePromptStudio(list, entity, state) {
  ...
  const derivationNote = state.isDefault
    ? `<div class="entity-state-generation-status independent"><b>BASE REFERENCE</b><span>Creates the primary approved design from the entity canon.</span></div>`
    : deriveReady
      ...
      : mode === "derive"
        ...
        : `<div class="entity-state-generation-status independent"><b>CREATE INDEPENDENTLY</b><span>Uses the entity canon and this state delta without an editable parent image.</span></div>`;
...
window.buildEntityStatePrompt = async (list, id, stateId, useLLM = false) => {
  ...
  if (!creationAssetDescription(list, entity).trim()) { toast("Describe the base entity first"); return null; }
...
function guidedBlockingPanel(s) {
  ...
  const blockingReview = typeof blockingAttemptReviewSummary === "function" ? blockingAttemptReviewSummary(s) : { rows, reviewed:0, recommended:null, recommendation:null, stale:false };
...
function guidedShotStatusCard(s, takes, neighbors) {
  ...
  const previewAction = preview ? `openMediaTheatre('${attr(encodeURIComponent(previewUrl))}','${attr(encodeURIComponent(previewTitle))}','${isVideo(previewTitle) ? "video" : "image"}')` : "";
  const media = preview
...
function guidedAudioPanel(s, c, profile, audioRefs) {
  ...
  return `<details class="guided-audio-block" ${guidedPanelOpen(s, "motionAudio", false) ? "open" : ""} ontoggle="rememberGuidedPanel('${s.id}','motionAudio',this.open)"><summary>Dialogue & voice <span>${audioRefs.length} linked</span></summary>
    <div class="guided-dialogue-grid">
      ...
      <label><span>Voice recording / identity</span><select onchange="setShotAudioField('${s.id}','voiceEntityId',this.value);setSimpleMotionAudio('${s.id}','voiceEntityId',this.value)"><option value="">No voice entity selected</option>${voiceOptions}</select></label>
...
function boundedShotTaskStatus(s, takes, taskId) {
  ...
  if (taskId === "inputs") return missingRefs ? { tone:"attention", label:`${missingRefs} missing` } : references.length ? { tone:"complete", label:"Ready" } : { tone:"pending", label:"Add inputs" };
  if (taskId === "look") return activeBlockingRow(s) ? { tone:"complete", label:"Guide active" } : blockingMediaRows(s).length ? { tone:"attention", label:"Choose guide" } : { tone:"pending", label:"Set look" };
  if (taskId === "frames") {
    if (activeRun) return { tone: ["failed","interrupted"].includes(activeRun.status) ? "attention" : "active", label: activeRun.status === "failed" ? "Failed" : activeRun.status === "awaiting-review" ? "Review" : "Running" };
    const states = progress.frames.map((frame,index) => guidedFrameStepState(s,frame,index,takes));
    if (states.some((row) => row.key === "review")) return { tone:"attention", label:"Review" };
    if (progress.requiredApproved) return { tone:"complete", label:"Approved" };
    return { tone:"pending", label:`${states.filter((row)=>row.key === "approved").length}/${states.length}` };
  }
  if (taskId === "motion") {
    if (["final","motion-approved"].includes(life.key)) return { tone:"complete", label:"Approved" };
    if (life.key === "review-motion") return { tone:"attention", label:"Review" };
    return progress.requiredApproved ? { tone:"pending", label:"Ready" } : { tone:"optional", label:"After frames" };
  }
  if (taskId === "deliver") return life.key === "final" ? { tone:"complete", label:"Final" } : ["motion-approved","still-ready"].includes(life.key) ? { tone:"pending", label:"Ready" } : { tone:"optional", label:"Later" };
...
function boundedShotTaskbarMarkup(s, takes, selectedId) {
  const defs = [
    ...
    ["look","Look & blocking","Camera, staging and authority review"],
  ...
  return `<nav class="focused-taskbar bounded-shot-taskbar clarity-taskbar" aria-label="Shot production stages">${defs.map(([id,label,detail]) => { const status=boundedShotTaskStatus(s,takes,id); return `<button type="button" class="focused-task-button tone-${status.tone} ${id===selectedId?"selected":""}" onclick="selectBoundedTask('shot-task','${attr(s.id)}','${id}')"><i></i><span><b>${esc(label)}</b><small>${esc(detail)}</small></span><em>${esc(status.label)}</em></button>`; }).join("")}</nav>`;
}
