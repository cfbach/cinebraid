  ...
  function motionSoundEditorForm(s, expanded = false) {
    ...
    return `<details class="motion-sound-composer" data-motion-unit="${attr(unit.id)}" ${expanded ? "open" : ""}><summary class="motion-sound-composer-summary"><div><span>MOTION & SOUND BRIEF · UNIT ${esc(unit.label || "A")}</span><b>Performance, camera, dialogue, and sound</b><small>This canonical brief compiles differently for each video model.</small></div><i>${esc(profile?.name || profileId)}</i></summary><div class="motion-sound-composer-body">
      <details open><summary>Performance & physical action <span>what moves</span></summary><div class="motion-sound-grid">
        ...
        <label><span>Emotion</span><input value="${attr(brief.performance.emotion)}" placeholder="frightened but controlled" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','emotion',this.value)"></label>
        <label><span>Facial performance</span><input value="${attr(brief.performance.facial)}" placeholder="jaw tightens, eyes track the cable" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','facial',this.value)"></label>
        <label><span>Body language</span><input value="${attr(brief.performance.bodyLanguage)}" placeholder="rigid shoulders, careful hands" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','bodyLanguage',this.value)"></label>
        <label><span>Gaze</span><input value="${attr(brief.performance.gaze)}" placeholder="from the seedling to the relay" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','gaze',this.value)"></label>
        <label class="wide"><span>Object interaction</span><input value="${attr(brief.performance.objectInteraction)}" placeholder="right hand seats the coupler while the left stabilizes the cable" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','objectInteraction',this.value)"></label>
        <label class="wide"><span>Secondary motion / environment beat</span><input value="${attr(brief.performance.secondaryMotion)}" placeholder="a status light goes dark after she stops" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','secondaryMotion',this.value)"></label>
        <label class="wide"><span>Required ending state</span><input value="${attr(brief.performance.endingState)}" placeholder="finish with her hand resting on the locked connector" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','endingState',this.value)"></label>
        <label class="wide"><span>Keep static</span><input value="${attr(brief.performance.staticConstraints)}" placeholder="room geometry, console position, character identity" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','staticConstraints',this.value)"></label>
        <label class="wide"><span>Prohibited movement / morphing</span><input value="${attr(brief.performance.prohibitedMotion)}" placeholder="no new gestures, no object morphing, no camera drift" onchange="setMotionSoundField('${s.id}','${unit.id}','performance','prohibitedMotion',this.value)"></label>
      </div></details>
      <details><summary>Camera & timing <span>how it is filmed</span></summary><div class="motion-sound-grid">
        <label class="wide"><span>Camera movement</span><input value="${attr(brief.camera.movement)}" placeholder="locked, slow push-in, gentle arc…" onchange="setMotionSoundField('${s.id}','${unit.id}','camera','movement',this.value)"></label>
        <label><span>Move timing</span><input value="${attr(brief.camera.timing)}" placeholder="begins at 1.0s, settles by 4.0s" onchange="setMotionSoundField('${s.id}','${unit.id}','camera','timing',this.value)"></label>
        <label><span>Framing behavior</span><input value="${attr(brief.camera.framing)}" placeholder="preserve medium close framing" onchange="setMotionSoundField('${s.id}','${unit.id}','camera','framing',this.value)"></label>
        <label class="wide"><span>Lens / focus behavior</span><input value="${attr(brief.camera.lensBehavior)}" placeholder="shallow focus shifts from hands to face" onchange="setMotionSoundField('${s.id}','${unit.id}','camera','lensBehavior',this.value)"></label>
      </div></details>
      <details class="motion-dialogue-editor" ${brief.dialogue.line ? "open" : ""}><summary>Dialogue & persistent voice <span>${brief.dialogue.line ? "locked line" : "optional"}</span></summary><div class="motion-sound-grid">
        ...
        <label><span>Line emotion</span><input value="${attr(brief.dialogue.emotion)}" placeholder="urgent but controlled" onchange="setMotionSoundField('${s.id}','${unit.id}','dialogue','emotion',this.value)"></label>
        <label><span>Delivery</span><input value="${attr(brief.dialogue.delivery)}" placeholder="quiet, breathless, slight pause" onchange="setMotionSoundField('${s.id}','${unit.id}','dialogue','delivery',this.value)"></label>
        ...
        <label><span>Start time</span><input value="${attr(brief.dialogue.startTime)}" placeholder="0.8s" onchange="setMotionSoundField('${s.id}','${unit.id}','dialogue','startTime',this.value)"></label>
        <label><span>End time</span><input value="${attr(brief.dialogue.endTime)}" placeholder="3.8s" onchange="setMotionSoundField('${s.id}','${unit.id}','dialogue','endTime',this.value)"></label>
      ...
      <details><summary>SFX, ambience & music <span>${(brief.sound.sfxEvents || []).length} timed event${(brief.sound.sfxEvents || []).length === 1 ? "" : "s"}</span></summary><div class="motion-sfx-list">${sfxRows(s,unit,brief)}</div><button class="ghost-btn" onclick="addMotionSoundSfx('${s.id}','${unit.id}')">＋ ADD SFX EVENT</button><div class="motion-sound-grid sound-tail">
        ...
        <label class="wide"><span>Required silence</span><input value="${attr(brief.sound.silence)}" placeholder="after the power drop, leave 0.5s of near-silence" onchange="setMotionSoundField('${s.id}','${unit.id}','sound','silence',this.value)"></label>
        <label class="wide"><span>Audio priorities</span><input value="${attr(brief.sound.priorities)}" placeholder="dialogue first, restrained machinery, no cinematic boom" onchange="setMotionSoundField('${s.id}','${unit.id}','sound','priorities',this.value)"></label>
      </div></details>
