...
function openModal(inner, options = {}) {
  ...
  const modalAnchor = MODAL_RETURN_FOCUS?.closest?.(".asset-creation-card, .entity-state-generation, .settings-block, .shot-main");
...
window.openMediaTheatre = (encodedUrl, encodedTitle = "Preview", kind = "", returnTo = "") => {
  ...
  const back = String(returnTo || "")
    ? `<button class="ghost-btn" data-theatre-return="1" onclick="window.inspectMedia(decodeURIComponent('${attr(encodeURIComponent(returnTo).replace(/'/g, "%27"))}'))">Back to inspector</button>`
    : "";
  const showTheatre = returnTo && document.querySelector("[data-media-inspector]") ? updateOpenModal : openModal;
  showTheatre(`<div class="media-theatre-modal" aria-label="Enlarged view of ${attr(title)}"><header><div><span>MEDIA PREVIEW</span><h3>${esc(title)}</h3><p>Large in-app inspection without leaving the shot workspace. Press Escape or Close to return.</p></div><button class="cancel" onclick="${returnTo ? `window.inspectMedia(decodeURIComponent('${attr(encodeURIComponent(returnTo).replace(/'/g, "%27"))}'))` : "closeModal()"}" aria-label="Close the enlarged view of ${attr(title)}">Close</button></header><div class="media-theatre-stage">${media}</div><footer><span>${mediaKind === "video" ? "Use the player controls to review motion and audio." : mediaKind === "audio" ? "Use the player controls to review the audio." : "The complete source image, fitted to the workspace without cropping."}</span><div class="media-theatre-footer-actions">${back}<a class="ghost-btn" href="${attr(url)}" target="_blank" rel="noopener">Open original</a></div></footer></div>`);
};
...
window.closeModal = (options = {}) => {
  ...
  setTimeout(() => {
    ...
    const anchor = returnFocus?.closest?.(".asset-creation-card, .entity-state-generation, .settings-block, .shot-main");
