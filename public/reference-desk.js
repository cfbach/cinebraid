...
(function() {
  ...
  const remainingWords=(n,word='required')=>(n===1?"One "+word+" view remains.":(COUNT_WORDS[n]||String(n))+" "+word+" views remain.");
  /* REFERENCE_FIRST_CANON_SIMPLIFICATION_V1 — what this reference needs NOW, read from the one reader
     every reference surface asks (entityImmediateNeed in entities.js). Unavailable, the Desk keeps its
     shipped behaviour exactly, and its coverage keeps saying "required". */
  const needOf=(list,entity)=>typeof entityImmediateNeed==='function'?entityImmediateNeed(list,entity):null;
  // The coverage surfaces name a view with its fraction spelled out (referenceViewLabel in entities.js).
  const viewWord=slot=>typeof referenceViewLabel==='function'?referenceViewLabel(slot):String(slot?.label||slot?.id||'');
  const needWordsOf=need=>need&&typeof entityImmediateNeedWords==='function'?entityImmediateNeedWords(need):null;
  ...
  function model(list,id) {
    ...
    const need=needOf(list,entity);
    // The Desk's own per-state count, named with the effective word the need reader gives it.
    const coverage={...R.coverage(entity,all,state.stateId),word:need?need.coverage.word:'required'};
    return {list,id,entity,state,currentState,allStates,media,all,strip,selected,row,truth,approved,linkedAvailable,coverage,need};
  }
  ...
  function view(list,id) {
    ...
    /* REFERENCE_FIRST_CANON_SIMPLIFICATION_V1 — ONE WARM ACTION, CHOSEN FROM THE NEED, IN THIS ORDER.
       1. An open decision on the image in view (approve, restore) keeps the accent, as EV2-7 decided.
       2. With no canon and nothing to decide, the first canon: Generate primary reference, on the canvas.
       3. Otherwise, the need's own next step on the needs strip (an owed continuity state, or the primary
          while another state is in view).
       4. Coverage carries it only when readiness could not clear it (the need's own build-coverage step).
          When nothing is needed now, planned coverage is optional work, so it stays reachable in its section
          as a secondary control and nothing on the Desk is warm: optional work must not look required. */
    const need=m.need, needWords=needWordsOf(need);
    const primaryId=need?need.standing.primaryId:R.defaultStateId(m.entity), onPrimary=m.state.stateId===primaryId;
    const decisionPending=!!selected?.available&&!sheet&&(m.row?.decision==='rejected'||!(m.truth.standing==='canon'&&m.truth.file===selected.name));
    const generateHere=!!need&&onPrimary&&!selected&&need.action.kind==='generate-primary';
    const stripAct=!!need&&!decisionPending&&(need.action.kind==='open-state'||(!onPrimary&&(need.action.kind==='generate-primary'||need.action.kind==='review-primary')));
    const coverageWarm=!decisionPending&&!generateHere&&!stripAct&&(!need||need.action.kind==='build-coverage');
    const linkedCopy=m.linkedAvailable.length===1?'An image is available in Production media but has not been added as a reference candidate.':m.linkedAvailable.length?m.linkedAvailable.length+' linked images are available in Production media. Choose which view each represents.':'';
    const ready=typeof falGenerationReady==='function'&&falGenerationReady();
    const op=typeof guidedPromptOp==='function'?guidedPromptOp('asset',list+':'+id,''):null, busy=op?.status==='busy', failed=op?.status==='error'&&op.action!=='improve'?String(op.error||''):'';
    /* The first canon, as one press. Generate compiles the rules-based prompt and opens the existing paid
       confirmation (generatePrimaryReference in creation-studio.js); nothing is sent before its own START
       GENERATION. Upload is the secondary path. Prompt editing, Braidy and Production media wait in one
       closed disclosure. With no provider configured, Upload carries the weight, as on the primary workspace. */
    const firstCanon='<div class="rd-empty rd-first-canon" data-rd-first-canon><span class="rd-empty-mark">◇</span><h2>No primary reference yet</h2><p>'+e(linkedCopy||('Generate '+(m.entity.name||id)+(/s$/i.test(m.entity.name||id)?'’':'’s')+' primary reference from its description, or upload an image you already have.'))+'</p><div class="rd-first-actions"><button type="button" id="rd-generate-primary" class="rd-button '+(ready?'rd-primary':'')+'" data-rd-action="generate-primary" aria-describedby="rd-first-note"'+(busy?' aria-disabled="true"':'')+'>'+(busy?'Preparing the prompt…':'Generate primary reference')+'</button>'+button('upload','Upload existing',ready?'':'rd-primary','id="rd-upload-existing"')+'</div><p class="rd-first-note" id="rd-first-note">'+e(ready?'Opens the paid confirmation with the prompt, model and settings. Nothing is sent or charged until you confirm there.':'No generation provider is set up here, so this prepares the prompt for use in another tool.')+'</p>'+(failed?'<p class="rd-first-error" role="alert">The prompt could not be prepared: '+e(failed)+'</p>':'')+'<details class="rd-more" id="rd-more"><summary>More options</summary><div class="rd-more-body">'+button('choose','Choose from Production media')+button('prompt','Edit the prompt, or use Braidy')+'</div></details></div>';
    const canvas=selected?.available?'<img id="rd-image" src="'+a(selected.url)+'" alt="'+a(m.entity.name+' — '+(m.currentState?.name||'Default'))+'"><p class="rd-image-error" hidden role="alert">This exact image is unavailable. It cannot be approved or used.</p>'+(sheet?'<figcaption class="rd-sheet-caption">Reference sheet · a source for views. Views are filled only when you assign a crop.</figcaption>':''):selected?'<div class="rd-empty"><span class="rd-empty-mark">◇</span><h2>Image unavailable</h2><p>The recorded candidate is preserved. CineBraid will not substitute another image.</p>'+button('refresh','Check availability')+'</div>':generateHere?firstCanon:'<div class="rd-empty"><span class="rd-empty-mark">◇</span><h2>No image assigned yet</h2><p>'+e(linkedCopy||'Choose an existing production image, or upload a new one to establish this reference.')+'</p><div>'+button('build','Build coverage',need?'':'rd-primary')+'</div></div>';
    ...
    const slots=views.map(({slot,v})=>{
      const label=viewWord(slot), img=v.item, present=v.present, note=v.label, hasResults=!results.known||results.has.has(slot.id);
    ...
    const missingViews=views.filter(x=>!x.v.filled), nextView=missingViews[0]?.slot||null, word=m.coverage.word;
    const coverageNext=!views.length
      ?'<p class="rd-coverage-state" data-rd-coverage-next="none">No '+word+' views are defined for this reference.</p>'+button('build','Build coverage')
      :nextView
      ?button('build',(m.coverage.filled?'Continue coverage — ':'Start coverage — ')+e(viewWord(nextView)),coverageWarm?'rd-primary':'','data-rd-build-slot="'+a(nextView.id)+'" data-rd-coverage-next="'+a(nextView.id)+'"')+'<small class="rd-coverage-remaining">'+e(remainingWords(missingViews.length,word))+'</small>'
      :'<p class="rd-coverage-state" data-rd-coverage-next="complete">All '+views.length+' '+word+' view'+(views.length===1?' is':'s are')+' filled for '+e(m.currentState?.name||'this continuity state')+'.</p>'+button('build','Build coverage');
    /* THE NEEDS STRIP. Needed now, coverage and continuity states on their own lines, each named for what it
       counts, with the need's own next step where that step is not already on the canvas or the decision bar. */
    const stripButton=stripAct?(need.action.kind==='open-state'?button('open-state','Open '+e(need.action.label)+' →','rd-primary','id="rd-need-action"'):button('show-primary','Show the primary reference','rd-primary','id="rd-need-action"')):'';
  ...
  function derivedViews(m,sheetItem) {
    ...
    return rows.filter(r=>r.coverageCrop?.sourceSheet===sheetItem.name && (r.targetStateId||def)===stateId).map(r=>{
      ...
      return {label:slot?viewWord(slot):(r.targetCoverageSlotName||r.targetCoverageSlotId||'View'),assigned};
    });
  }
  function library(tab='all') {
    ...
    return '<section class="rd-library" data-reference-library><nav class="rd-crumb"><a href="#/production">Production</a><span>/ References</span></nav><header class="rd-heading"><div><p class="rd-eyebrow">Production continuity</p><h1>References</h1><p>Choose the images your production can rely on.</p></div>'+button('new','Add reference','rd-primary')+'</header><div class="rd-library-tools"><nav aria-label="Reference types">'+[['all','All'],['characters','Characters'],['locations','Locations'],['props','Props'],['vehicles','Vehicles'],['canon','Approved'],['audio','Audio']].map(([id,label])=>'<a class="'+(id===selected?'selected':'')+'" href="#/library/'+id+'">'+label+'</a>').join('')+'</nav><label class="rd-search">Find a reference<input id="rd-search" type="search" placeholder="Search names or reference IDs"></label></div><div class="rd-library-grid">'+entries.map(({list,entity})=>{
      ...
      /* REFERENCE_FIRST_CANON_SIMPLIFICATION_V1 — each card says what THIS reference needs now, from the same
         reader as its page, and names its coverage as coverage. "0 of 4 required views filled" on every card
         read as eighteen backlogs; the production is waiting on none of those views. */
      const need=needOf(list,entity),words=needWordsOf(need);
      const coverage={...R.coverage(entity,media,(ss.find(s=>s.isDefault)||ss[0])?.id||'state-default'),word:need?need.coverage.word:'required'};
      const needLine=words?'<span class="rd-card-need'+(words.count?' is-owed':'')+'" data-rd-card-need="'+words.count+'">Needed now · '+e(words.now)+'</span>':'';
      const coverageText=words?'Coverage · '+coverage.filled+'/'+coverage.required+' '+coverage.word+' view'+(coverage.required===1?'':'s'):coverage.filled+' of '+coverage.required+' required views filled';
      return '<a class="rd-library-card" data-rd-search="'+a((entity.name+' '+entity.id).toLowerCase())+'" href="'+routeFor(list,entity.id)+'"><div class="rd-library-image">'+(preview?'<img src="'+a(preview.url)+'" alt="">':'<span>'+ (approved?'Approved image unavailable':'No image assigned yet')+'</span>')+'</div><div><small>'+e(types[list])+'</small><h2>'+e(entity.name||entity.id)+'</h2><p>'+e(approved?(media.some(m=>m.name===approved.file)?'Approved reference':'Approved image unavailable'):media.length?media.length+' candidate'+(media.length===1?'':'s')+' · review needed':'Choose or upload an image')+'</p>'+needLine+'<span class="rd-card-coverage">'+e(coverageText)+'</span></div></a>';
  ...
  /* EV2-7 — the coverage target in plain words, refreshed whenever the state or view changes. Display only: the picker still assigns through the recorded ids. */
  function pickerTargetWords(p){
    const entity=P[p.list]?.find(x=>x.id===p.id),state=entity&&collection(entity).find(s=>s.id===p.stateId),slot=entity&&(entity.coverageSlots||[]).find(s=>s.id===p.slotId);
    p.entityName=entity?.name||p.id;p.stateName=state?(state.name||state.id):'';p.slotLabel=slot?viewWord(slot):'';
    // The default state, so an image that records no state of its own is exact for the default view and an earlier selection anywhere else.
    p.defaultStateId=entity?R.defaultStateId(entity):'';return p;
  }
  async function openPicker(slotId='',upload=false,options={}) {
    ...
    picker={list:m.list,id:m.id,slug:ACTIVE_PROJECT_SLUG,epoch:PROJECT_OPEN_EPOCH,revision:PROJECT_REVISION,stateId:m.state.stateId,slotId,images:[],selected:'',busy:false,upload,file:null,error:'',onDone:options.onDone||null,onCancel:typeof options.onCancel==='function'?options.onCancel:null,sheetMode:!upload&&options.sheetMode===true};
    // The library opens on this target's own category, with the exact target first. The filmmaker can still choose All or any other category.
    picker.discovery=CineBraidMediaDiscovery.defaults(true,pickerTargetWords(picker));
  ...
  function renderPicker(focus='rd-picker-cancel') {
    ...
    const p=pickerTargetWords(picker),entity=P[p.list]?.find(e=>e.id===p.id),selected=p.images.find(r=>r.assetId===p.selected),state=collection(entity).find(s=>s.id===p.stateId),slot=(entity.coverageSlots||[]).find(s=>s.id===p.slotId);
  ...
  function syncPicker() {
    ...
    if(summary)summary.innerHTML='<b>'+e(slot?viewWord(slot):'Choose a required view')+'</b><p>'+e(state?.name||'Choose a continuity state')+'</p><small>'+(slot&&R.selectedKey(entity,slot,p.stateId)?'This replaces the selected image for this view. The previous candidate and any approval are retained.':'This adds a candidate and fills the selected view.')+' Approval remains a separate decision.</small>';
  ...
  function action(id,slotId='') {
    ...
    /* REFERENCE_FIRST_CANON_SIMPLIFICATION_V1. Generate composes the two shipped actions (compile, then the
       paid confirmation) and submits nothing itself. Focus is held on the button the filmmaker pressed through
       the redraws the compile causes, and returns to it when the confirmation closes. */
    if(id==='generate-primary'){
      if(typeof generatePrimaryReference!=='function')return toast('Primary reference generation is unavailable on this page. Reload CineBraid.');
      const hold=()=>document.getElementById('rd-generate-primary')||document.getElementById('rd-status');
      return generatePrimaryReference(m.list,m.id,{beforeConfirm:()=>hold()?.focus({preventScroll:true}),resolveReturnFocus:hold});
    }
    if(id==='prompt')return window.openPrimaryPromptWorkspace?.(m.list,m.id);
    if(id==='open-state'&&m.need?.action.kind==='open-state')return window.openEntityStateNeed?.(m.list,m.id,m.need.action.stateId);
    if(id==='show-primary'){Object.assign(local(m.list,m.id),{stateId:m.need?.standing.primaryId||R.defaultStateId(m.entity),selected:''});return repaint('rd-state');}
  ...
  document.addEventListener('click',event=>{
    /* The desk's direct way into a reference's production needs: the tools page opens on
       that task, written through the same focused-task key its own task bar writes. */
    const needs=event.target.closest('a[data-rd-tools-task]');
    if(needs&&active&&typeof boundedWriteFocusedTask==='function')boundedWriteFocusedTask('entity-task',active.list+':'+active.id,needs.dataset.rdToolsTask);
  ...
  document.addEventListener('change',event=>{
    ...
    // A new state or view is a new target: the header and the exact-target order follow it, and focus stays on the select that changed.
    if(['rd-assign-state','rd-assign-slot'].includes(t.id)&&!picker.upload)renderPicker(t.id);
