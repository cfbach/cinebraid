...
(function() {
  ...
  function open({list,id,stateId,slotId='',method='',source=null}={}) {
    ...
    build={token:++serial,slug:ACTIVE_PROJECT_SLUG,epoch:PROJECT_OPEN_EPOCH,revision:PROJECT_REVISION,list,id,stateId,stateName:ctx.state.name||'Default',isDefault:ctx.isDefault,
      ...
      assign:{status:'idle',error:'',code:'',bindingId:''},generation:{receiptId:ctx.receiptId,slotIds:ctx.missing.map(s=>s.id),clientRequestIds:{},status:'idle',error:'',jobIds:[],confirm:false},
  ...
  function stepGenerate(b,ctx,entity) {
    const reason=generateReason(b,ctx,entity),chosen=ctx.missing.filter(s=>b.generation.slotIds.includes(s.id)),n=chosen.length;
    const price=typeof generationPriceLine==='function'&&typeof configuredImageRate==='function'?generationPriceLine({rate:configuredImageRate(typeof CONFIG==='object'?CONFIG:{}),quantity:3*n,local:false}):null;
    ...
    if(g.confirm)return '<div class="bc-confirm" role="alert"><p><b>This will submit '+n+' paid requests and may return up to '+3*n+' images.</b></p></div><div class="bc-row">'+button('data-bc-action="generate-back"','Back','',b.busy?'disabled':'')+button('data-bc-action="generate-submit"','Submit '+n+' paid requests','rd-primary',b.busy?'disabled':'')+'</div>';
    return (reason?'<p class="bc-error" role="alert">'+e(reason)+'</p>':'')
      ...
      +'<fieldset class="bc-gen-views"><legend>Views to generate</legend>'+ctx.missing.map(s=>'<label class="rd-check"><input type="checkbox" id="bc-gen-'+a(s.id)+'" data-bc-gen-slot="'+a(s.id)+'" '+(b.generation.slotIds.includes(s.id)?'checked':'')+' '+(b.busy?'disabled':'')+'> '+e(s.label||s.id)+'</label>').join('')+'</fieldset>'
      +'<div class="bc-quote"><b>'+plural(n,'paid request')+' · up to '+plural(3*n,'image')+'</b><span>Cloud · fal (existing coverage owner). Each request is paid.</span>'+(price?'<em data-bc-price-kind="'+a(price.kind)+'">'+e(price.headline)+' · '+e(price.detail)+'</em>':'')+'</div>'
      +(g.error?'<p class="bc-error" role="alert">'+e(g.error)+'</p>':'')
      +'<div class="bc-row">'+button('data-bc-action="back"','Back','',b.busy?'disabled':'')+button('data-bc-action="generate-submit"',b.busy?'Preparing… nothing has been sent yet':g.status==='failed'?'Retry: submit '+n+' paid request'+(n===1?'':'s'):'Submit '+n+' paid request'+(n===1?'':'s'),'rd-primary',(reason||!n||b.busy)?'disabled':'')+'</div>';
  }
  ...
  async function submitGeneration(b) {
    ...
    if(count>2&&!g.confirm){g.confirm=true;return render();}
  ...
  function act(t) {
    ...
    if(x==='next'){
      ...
      Object.assign(b,{slotId:next?.id||b.slotId,pendingCrop:null,assign:{status:'idle',error:'',code:'',bindingId:''},returned:{name:'',ack:false,error:''},notice:'',step:'method'});
  ...
  document.addEventListener('change',event=>{
    ...
    if(t.id==='bc-view'){b.slotId=t.value;b.returned={name:'',ack:false,error:''};return render({focus:'bc-view'});}
