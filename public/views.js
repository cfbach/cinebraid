...
const ROUTES = {
  ...
  async settings() {
    ...
    const projectPanel = `<section class="settings-block"><div class="settings-title-row"><div><h3>Project preferences</h3><p class="hint">These details belong to the project record, which saves itself a moment after every edit — the same way the rest of CineBraid saves your work. There is nothing to press, and the line below always states where the last edit stands.</p></div></div><div class="two-col">
      ...
      ${field("Workspace emphasis", `<select id="cfg-project-emphasis" onchange="studioSetProjectEmphasis(this.value)"><option value="manual" ${projectWorkflowEmphasis() === "manual" ? "selected" : ""}>Manual-first production</option><option value="assisted" ${projectWorkflowEmphasis() === "assisted" ? "selected" : ""}>Assisted generation</option></select><span class="hint">This changes hierarchy and defaults, not project data.</span>`)}
    ...
    </div><div class="settings-save-line"><span class="settings-save-state" data-mirror-save-state="1" data-state="saved" role="status">Saved automatically</span><small class="settings-save-model">Project details save themselves; the two buttons below only produce copies.</small></div><div class="settings-actions"><a class="ghost-btn" href="#/settings/project-recovery">Project backups & recovery</a><button class="add-btn" onclick="doExport()">Export project</button><button class="ghost-btn" onclick="downloadJSON()">Download JSON backup</button><span id="export-note" class="hint"></span></div></section>`;
