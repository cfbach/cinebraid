...
const ROUTES = {
  ...
  async settings() {
    ...
    const continuityProviderFields = `<div class="two-col"><div class="settings-subheading"><span>Continuity analysis</span><small>Checks declared references frame by frame. Configured on its own because it sends exactly one image per request — general vision settings do not apply to it.</small></div>${field("Continuity provider", `<select id="cfg-continuity-provider"><option value="" ${continuityConfig.visionProvider ? "" : "selected"}>Not configured — continuity checks stay off</option><option value="custom" ${continuityConfig.visionProvider === "custom" ? "selected" : ""}>Custom AI server</option><option value="openai" ${continuityConfig.visionProvider === "openai" ? "selected" : ""}>OpenAI API</option></select>`)}${field("Continuity model", `<input id="cfg-continuity-model" value="${attr(continuityConfig.visionModel || "")}" placeholder="the provider's own vision model"><span class="hint">Leave blank to use the provider's configured vision model.</span>`)}</div>`;
