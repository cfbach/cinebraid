...
const ROUTES = {
  ...
  scene(id) {
    ...
    return `<div class="crumb"><a href="#/shots/scenes">PRODUCTION</a> / ${esc(sc.id)}</div>
    ...
    <div class="two-col" style="margin-top:16px">
      ${field("What happens — the beat (sacred)", ta(sc, "whatHappens", "scenes", sc.id))}
  ...
  async settings() {
    ...
    const coreTabs = tabs.slice(0, 4).map(tabButton).join("");
    const serviceTabs = tabs.slice(4).map(tabButton).join("");
    const tabbar = `<nav class="settings-nav-shell" aria-label="Settings sections"><section><span>WORKSPACE</span><div class="settings-tabs">${coreTabs}</div></section><section><span>OPTIONAL ASSISTED SERVICES</span><div class="settings-tabs">${serviceTabs}</div></section></nav>`;
    const appearancePanel = `<section class="settings-block brand-settings"><div class="settings-title-row"><div><h3>Website-aligned appearance</h3><p class="hint">Restore the CineBraid brand controls and preview them live.</p></div><div class="settings-inline-actions"><button class="ghost-btn" onclick="previewAppearanceSettings()">Preview</button><button class="ghost-btn" onclick="resetAppearanceSettings()">Reset</button></div></div><div class="two-col">
      ...
      ${field("Interface scale", `<input id="cfg-ui-scale" type="range" min="90" max="110" step="5" value="${attr(Number(appearance.scale || 100))}" oninput="document.getElementById('cfg-ui-scale-readout').textContent=this.value + '%' ; previewAppearanceSettings()"><div class="range-readout" id="cfg-ui-scale-readout">${esc(String(Number(appearance.scale || 100)))}%</div>`)}
      ...
      <div class="settings-preview-card"><span>Brand preview</span><b>Keep every shot tied to the approved truth.</b><small>Current accent and surface styling update immediately in this session.</small></div>
    </div><div class="settings-actions"><button class="add-btn" onclick="saveAppearanceSettings()">Save appearance settings</button></div></section>`;
    const filesPanel = `<section class="settings-block"><div class="settings-title-row"><div><h3>Files & storage</h3><p class="hint">These paths are now active backend controls. Changing the project root safely copies missing project files before switching.</p></div><button class="ghost-btn settings-head-button" onclick="refreshWorkspaceStatus()">Check active paths</button></div><div class="two-col">
      ${field("Project root", `<input id="cfg-project-root" value="${attr(workspace.projectRoot || "")}" placeholder="/Projects/CineBraid">`)}
      ${field("Media root", `<input id="cfg-media-root" value="${attr(workspace.mediaRoot || "")}" placeholder="/Projects/CineBraid/media">`)}
      ${field("Output folder", `<input id="cfg-output-root" value="${attr(workspace.outputRoot || "")}" placeholder="/Projects/CineBraid/exports">`)}
      ${field("Backup folder", `<input id="cfg-backup-root" value="${attr(workspace.backupRoot || "")}" placeholder="/Projects/CineBraid/backups">`)}
    ...
    </div><div id="workspace-settings-note" class="workspace-settings-note">Project root controls all project records and media. Assisted sync copies new files from the configured media root without overwriting existing files.</div><div class="settings-actions"><button class="add-btn" onclick="saveWorkspaceSettings()">Apply storage settings</button></div></section>`;
    const namingPanel = `<section class="settings-block"><div class="settings-title-row"><div><h3>Naming & organization</h3><p class="hint">Bring back explicit filename rules for approvals and exports.</p></div></div><div class="two-col">
    ...
    </div><div class="filename-preview"><span>Example preview</span><code id="cfg-filename-preview">${esc(["SIGNAL_BLOOM","SH013","PRIMARY","V003.png"].join("_"))}</code></div><div class="settings-actions"><button class="add-btn" onclick="saveWorkspaceSettings()">Save naming settings</button></div></section>`;
    const projectPanel = `<section class="settings-block"><h3>Project</h3><div class="two-col">
      ${field("Title", `<input value="${attr(P.meta.title)}" onchange="P.meta.title=this.value;dirty();load()">`)}
      ${field("Format", `<input value="${attr(P.meta.format || "")}" onchange="P.meta.format=this.value;dirty()">`)}
      ${field("Default aspect ratio", `<input value="${attr(P.meta.aspectRatio || "")}" placeholder="2.39:1" onchange="setGlobalCreationField('aspectRatio',this.value)">`)}
      ...
      ${field("Prompt history retention", `<input type="number" min="1" max="100" value="${attr(P.meta.promptBuildRetention || 12)}" onchange="P.meta.promptBuildRetention=Math.max(1,Math.round(Number(this.value)||12));applyPromptBuildRetention(P);dirty();route()"><span class="hint">Approved and linked builds are protected.</span>`)}
    ...
    </div><div class="settings-actions"><button class="add-btn" onclick="doExport()">Export project</button><button class="ghost-btn" onclick="downloadJSON()">Download JSON backup</button></div></section>`;
