...
const ROUTES = {
  ...
  async settings() {
    const selected = studioSettingsSelection();
    const [c, health, backupData, accountData] = await Promise.all([
      fetch("/api/config").then((r) => { if (!r.ok) throw new Error("Settings could not be read. Retry without changing saved values."); return r.json(); }),
      selected === "recovery" || selected === "project-recovery" ? fetch("/api/system/health").then((r) => r.json()).catch(() => ({})) : {},
      selected === "project-recovery" ? projectBackupList() : {},
      fetch("/api/accounts").then((r) => { if (!r.ok) throw new Error("Account read failed"); return r.json(); }).then((data) => Array.isArray(data.accounts) && Array.isArray(data.providers) ? data : { error: true }).catch(() => ({ error: true })),
    ]);
    CONFIG = c || {};
    applyTheme();
    ...
    const appearancePanel = `<section class="settings-block brand-settings"><div class="settings-title-row"><div><h3>Appearance</h3><p class="hint">Changing any control below repaints the workspace immediately so you can judge it at full size. That is a preview only — saving stores the application appearance. Unsaved edits stay in this page session; leaving Settings ends the preview.</p></div><div class="settings-inline-actions"><button class="ghost-btn" onclick="resetAppearanceSettings()">Load defaults</button><button class="ghost-btn" onclick="discardAppearancePreview()">Discard preview</button></div></div><div class="two-col">
      ...
      ${field("Surface theme", `<select id="cfg-theme-surface" onchange="previewAppearanceSettings()"><option value="night" ${(appearance.surface || "night") === "night" ? "selected" : ""}>Dark studio</option><option value="graphite" ${appearance.surface === "graphite" ? "selected" : ""}>Graphite studio</option><option value="cool" ${appearance.surface === "cool" ? "selected" : ""}>Website navy</option><option value="warm" ${appearance.surface === "warm" ? "selected" : ""}>Warm studio</option><option value="light" ${appearance.surface === "light" ? "selected" : ""}>Light canvas</option></select>`)}
    ...
    </div><div class="settings-actions"><button class="add-btn" onclick="saveAppearanceSettings()">Save appearance</button>${panelState("preview", "Save records application appearance and updates this browser.")}</div></section>`;
    /* THE PANEL SAYS WHERE THE PROJECTS ACTUALLY ARE, not just what is typed in the box.
       The box holds a SAVED OVERRIDE and is empty on most installs, so reading it tells
       a person nothing about where their work lives — which is exactly the confusion
       that let productions sit inside the application folder unnoticed. The state line
       below is filled from /api/workspace/status the moment this view renders, and it
       names the resolved path, where that path came from, and whether it is somewhere
       an update can overwrite. */
    const filesPanel = `<section class="settings-block"><div class="settings-title-row"><div><h3>Files & storage</h3><p class="hint">These folders control where your project records, media, exports and backups are kept. Changing the project folder copies any missing project files across before it switches, and leaves the originals exactly where they are.</p></div><button class="ghost-btn settings-head-button" onclick="refreshWorkspaceStatus()">Check active paths</button><button class="ghost-btn settings-head-button" onclick="openCineBraidProjectFolder()">Open project folder</button></div><p id="workspace-root-state" class="hint workspace-root-state" role="status">Checking where projects are kept…</p><details class="settings-local-storage" id="settings-location"><summary>Where CineBraid keeps its settings</summary><p id="settings-location-detail" class="hint">Checking…</p></details><div class="two-col">
      ${field("Project root", `<input id="cfg-project-root" value="${attr(workspace.projectRoot || "")}" placeholder="Leave blank to use the CineBraid default folder"><span class="hint">Leave this empty and CineBraid keeps projects in its default folder outside the application. <button type="button" class="text-link-btn" onclick="useRecommendedProjectRoot()">Fill in the recommended folder</button></span>`)}
      ...
      ${field("Sync mode", `<select id="cfg-sync-mode"><option value="manual" ${(workspace.syncMode || "manual") === "manual" ? "selected" : ""}>Manual sync</option><option value="assisted" ${workspace.syncMode === "assisted" ? "selected" : ""}>Assisted import when syncing</option></select><span class="hint">Assisted sync copies new media when sync runs; this is not a background folder watcher.</span>`)}
    ...
    /* NO_PROJECT_SHELL_TRUTH_V1 — A PROJECT-SCOPED PANEL REFUSES RATHER THAN RENDERS.
       Settings is reachable before a first project exists, which is the whole point:
       Connections and the studio-wide sections are how a filmmaker sets CineBraid up.
       Two sections are not studio-wide — these preferences are stored in the project
       record, and backups are backups OF a project — so they say so, on the panel the
       person went looking for, rather than being built from a record that is not
       there. The form is not drawn at all: a field that cannot be saved is a field
       that should never have accepted a keystroke. */
    const projectScopeRefusalReason = typeof shellSettingsScopeAvailability === "function"
      ? shellSettingsScopeAvailability("project", { hasProject: false }).reason
      : "These settings are stored in the project record, and no project is open.";
    const projectScopeRefusal = (title, detail) => `<section class="settings-block settings-project-refusal" data-settings-refusal="no-project" role="status"><div class="settings-title-row"><div><h3>${esc(title)}</h3><p class="hint">${esc(detail)}</p></div></div><p class="hint">${esc(projectScopeRefusalReason)}</p><div class="settings-actions"><button class="add-btn" onclick="newProject()">Create a project</button><a class="ghost-btn" href="#/settings/connections">Set up connections instead</a></div><div class="settings-save-line"><span class="settings-save-state" data-state="none" role="status">Nothing on this panel can be saved yet.</span><small class="settings-save-model">These values are stored in a project record. Application-wide settings are unaffected and remain editable.</small></div></section>`;
    const projectPanel = !P ? projectScopeRefusal("Project preferences", "Title, format, default aspect ratio, workspace emphasis and prompt retention are saved with a project.") : `<section class="settings-block"><div class="settings-title-row"><div><h3>Project preferences</h3><p class="hint">These details belong to the project record, which saves itself a moment after every edit — the same way the rest of CineBraid saves your work. There is nothing to press, and the line below always states where the last edit stands.</p></div></div><div class="two-col">
      ${field("Title", `<input id="cfg-project-title" value="${attr(P.meta.title)}" onchange="setProjectTitle(this.value)">`)}
      ${field("Format", `<input id="cfg-project-format" list="cinebraid-format-presets" value="${attr(P.meta.format || "")}" placeholder="Short film" onchange="P.meta.format=this.value;dirty()"><datalist id="cinebraid-format-presets">${PROJECT_FORMAT_PRESETS.filter(([value]) => value).map(([value, label]) => `<option value="${attr(value)}">${esc(label)}</option>`).join("")}</datalist><span class="hint">What this project is being made as. The same suggestions the new-project screen offers, and any other wording is still accepted.</span>`)}
      ${field("Default aspect ratio", `<input id="cfg-project-aspect" list="cinebraid-aspect-presets" value="${attr(P.meta.aspectRatio || "")}" placeholder="2.39:1" onchange="setGlobalCreationField('aspectRatio',this.value)"><datalist id="cinebraid-aspect-presets">${CINEBRAID_ASPECT_PRESETS.map(([value, label]) => `<option value="${attr(value)}">${esc(label)}</option>`).join("")}</datalist><span class="hint">The format every shot is judged and generated in unless a shot overrides it. Any width:height is accepted.</span>`)}
      ${field("Workspace emphasis", `<select id="cfg-project-emphasis" onchange="studioSetProjectEmphasis(this.value)"><option value="manual" ${projectWorkflowEmphasis() === "manual" ? "selected" : ""}>Manual-first production</option><option value="assisted" ${projectWorkflowEmphasis() === "assisted" ? "selected" : ""}>Assisted generation</option></select><span class="hint">Saved with this project. Changes which tools lead and their defaults; it does not change shots, media or approvals.</span>`)}
      ${field("Prompt versions kept per shot", `<input id="cfg-project-retention" type="number" min="1" max="100" value="${attr(P.meta.promptBuildRetention || 12)}" onchange="P.meta.promptBuildRetention=Math.max(1,Math.round(Number(this.value)||12));applyPromptBuildRetention(P);dirty()"><span class="hint">Older prompt versions are trimmed past this number. Approved and linked prompts are never trimmed.</span>`)}
    ...
    </div><div class="settings-save-line"><span class="settings-save-state" data-mirror-save-state="1" data-state="saved" role="status">Saved automatically</span><small class="settings-save-model">Project details save themselves. Backups and export copies are under Recovery &amp; export.</small></div></section>`;
    ...
    const tierOf = (id) => (ASSISTANT_TIERS.find(([, , , ids]) => ids.includes(id)) || [""])[0];
    const tierRuntimes = (tier) => (ASSISTANT_TIERS.find(([id]) => id === tier) || [, , , []])[3];
    /* Choosing a tier lands on that tier's first runtime unless the current one is
       already inside it, so a filmmaker moving Cloud -> Local is not left on a
       provider that belongs to neither. */
    const tierEntryProvider = (tier, current) => (tierRuntimes(tier).includes(current) ? current : tierRuntimes(tier)[0]);
    ...
    const endpointHost = (url) => {
      try { return new URL(String(url || "")).host; } catch { return url ? "Check endpoint format" : ""; }
    };
    ...
    /* C2 — THE STATUS WORD COMES FROM THE AUTHORITATIVE STANDING.
     *
     * A1 had this card read "Off" for a blank model as well as for a disabled
     * provider, on the reasoning that no image is read either way. That reasoning
     * was wrong about one case and it made this panel a fourth place that decided
     * what Off means: an OpenAI provider with a key and no explicit model is
     * served from the provider default and the server reports it READY, while this
     * card called it Off — the same contradiction, on the screen a filmmaker opens
    ...
    const visionStatus = { off: "Off", checking: "Status unknown", ready: "Configured", unavailable: "Unavailable" }[visionState];
    ...
    /* The standalone "Braidy is off" hint and the "Turn Braidy off" button are both
       ...
       a hint restating a state the control already shows, are exactly what the
       two-step shape removes. Turning Braidy off is the same single call it was. */
    const braidyConfigure = `${braidyProviderChoice}<p class="hint">Choosing a provider changes Braidy's saved selection immediately. It sends no test prompt. Existing readiness refresh may read runtime model inventories with stored credentials, without project material.</p>${provider !== "none" ? `<a class="ghost-btn" href="#/settings/assistant-${attr(provider)}">Open ${esc(providerLabel(provider))} connection & models</a>` : `<a class="ghost-btn" href="#/settings/connections">Set up a connection</a>`}`;
    ...
    const generationPanel = `<section class="settings-block fal-settings"><div class="settings-title-row"><div><h3>Generation defaults</h3><p class="hint">Existing provider-specific defaults for new jobs. A shot operation makes its own choice; changing these values does not change past results. Keys and account setup live in Connections.</p></div><button class="ghost-btn" onclick="testFalGenerationConnection()">Inspect saved Fal configuration</button></div><p class="hint"><a href="#/settings/fal">Fal connection</a> · OpenAI image models here run through Fal; they are not a separate OpenAI connection.</p><label class="checkline"><input id="cfg-fal-enabled" type="checkbox" ${fal.enabled ? "checked" : ""}> Enable FAL image and video generation</label><div class="two-col">
      ...
      </div><details><summary>Advanced · saved motion adapter endpoints</summary><div class="two-col"><div class="settings-subheading"><span>MiniMax H3 video endpoints</span><small>Saved adapter endpoints. Availability is not checked by saving a default.</small></div>
      ...
      </div></details><div class="two-col">${field("H3 default resolution", `<select id="cfg-fal-h3-resolution"><option value="2K" ${(fal.h3Resolution || "2K") === "2K" ? "selected" : ""}>2K</option><option value="768P" ${fal.h3Resolution === "768P" ? "selected" : ""}>768P</option></select>`)}
      ...
      ${field("Estimated cost per image (USD) · your estimate", `<input id="cfg-fal-cost-per-image" type="number" min="0" max="100" step="0.001" value="${attr(Number(fal.estimatedCostPerImage || 0))}" placeholder="0.00"><small>This is your saved estimate, not a provider quote or account balance. Zero means no usable estimate.</small>`)}
      ...
      /* Provider defaults reference the existing account owner; they never carry a credential. */
    ...
    </div><p id="civitai-grants-note" class="hint" role="status"></p><p class="hint">Check model makes a read-only request to Civitai using the saved account credential. It does not generate, grant spending permission or establish a balance.</p><div class="settings-actions"><button class="ghost-btn" onclick="checkCivitaiResource()">Check model with saved Civitai account</button><span id="civitai-resource-note" role="status" class="settings-state-chip" data-tone="${civ.enabled ? "attention" : "off"}">${civ.enabled ? "Not checked yet — press Check model" : "Civitai generation is off"}</span></div></section>`;
    })()}<div class="settings-actions"><button class="add-btn" onclick="saveConfig('generation')">Save generation settings</button><span id="fal-test-note" role="status" class="settings-state-chip" data-tone="${fal.enabled ? (fal.apiKey || fal.keySource === "environment" ? "ready" : "attention") : "off"}">${fal.enabled ? (fal.apiKey || fal.keySource === "environment" ? "Saved Fal key is present; reachability and balance unknown" : "FAL generation is on, but needs a key") : "FAL generation is off"}</span>${panelState("manual", "Save records these defaults; nothing is generated and nothing is charged.")}</div><p class="hint fal-security-note">Paid generation is only submitted after confirmation.</p></section>`;
    /* Local runtime setup keeps the existing config owner, loopback boundary and workflow registry. */
    const comfy = c.generation?.comfy || {};
    const integrationsPanel = `<section class="settings-block comfy-settings" data-integration="comfyui"><div class="settings-title-row"><div><h3>Local ComfyUI</h3><p class="hint">Run your own ComfyUI workflows from a shot. CineBraid connects to a ComfyUI on this machine, sends the shot's prompt and reference images into a workflow you have mapped, and brings the result back as a candidate to review. This adapter accepts only a runtime on this computer. A connection does not select it for a job or approve its results.</p></div><button class="ghost-btn" onclick="testComfyConnection()">Check saved local runtime</button></div><label class="checkline"><input id="cfg-comfy-enabled" type="checkbox" ${comfy.enabled ? "checked" : ""}> Generate with a local ComfyUI</label><div class="two-col">
    ...
    </div><div class="settings-actions"><button class="add-btn" onclick="saveConfig('integrations')">Save ComfyUI settings</button><span id="comfy-test-note" role="status" class="settings-state-chip" data-tone="${comfy.enabled ? "attention" : "off"}">${comfy.enabled ? "Reachability unknown — check saved runtime" : "Local ComfyUI is off"}</span>${panelState("manual", "Save records the address and folder; nothing is generated and nothing is charged.")}</div>
    ...
    <p class="hint comfy-cost-note">This check reads the saved local runtime and workflow metadata; it submits no generation. Inspect workflows before running them: custom nodes may contact external services or incur charges. An offline local runtime never selects a cloud fallback.</p></section>`;
    /* Safe account projections keep identity, verification and spending permission separate. */
    ...
    const accountStateWords = (row) => (
      row.status === "connected" ? (row.stale ? "Identity verified previously — recheck needed" : "Identity verified — not a live availability check")
    ...
    const accountsPanel = `<section class="settings-block account-settings"><div class="settings-title-row"><div><h3>Civitai accounts</h3><p class="hint">Connect the services you already have an account with. Credentials stay on this CineBraid server; only safe account identity and status are shown. Connecting an account generates nothing and spends nothing.</p></div></div><p class="hint">Account identity, verification and generation permission are separate. Connecting or rechecking contacts Civitai using your credential and may update saved verification metadata; it does not submit generation. CineBraid does not read your Buzz balance. A model check is separate from identity verification.</p><p class="hint">Civitai generation sends selected prompts and material to the cloud after confirmation and may spend Buzz. Check your provider agreement for rights, retention, regional access and privacy; these are not verified here.</p><p id="civitai-grants-note" class="hint" role="status"></p><div class="account-list">${accountData.error ? `<p class="hint" role="status">Account status could not be read. Saved accounts have not been removed. <button class="ghost-btn" onclick="route()">Retry account list</button></p>` : accountProviders.map((provider) => {
    ...
    </div><div class="settings-actions"><button class="add-btn" onclick="saveConfig('accounts')">Save account settings</button><span id="account-note" class="hint" role="status"></span>${panelState("manual", "Save records the application ID; connecting an account is the button above.")}</div></section>`;
    const recoveryPanel = !P ? projectScopeRefusal("Project backups & export", "Rotating backups, restore points and export copies all belong to one project.") : `<section class="settings-block project-recovery"><div class="settings-title-row"><div><h3>Project backups & recovery</h3><p class="hint">Rotating backups, diagnostics and support records.</p></div><button class="ghost-btn" onclick="createManualProjectBackup()">Create backup now</button></div><div class="settings-health-grid"><article><span>Server</span><b>${health.ok === true ? "Responding" : health.ok === false ? "Needs attention" : "Unknown"}</b><small>${esc(health.message || health.status || "Local CineBraid service")}</small></article><article><span>Project folder</span><b>${esc(activeProjectSlug() || "—")}</b><small>The folder name this project is stored under. Use Reports for integrity checks and the project log.</small></article></div><div id="project-backup-note" class="hint"></div>${backupData.error ? `<p class="hint backup-list-error" role="alert">${esc(backupData.error)}</p>` : (backupData.backups || []).length ? `<div class="project-backup-list">${backupData.backups.map((item) => `<article><div><b title="${attr(item.name)}">${esc(item.name)}</b><small>${esc(new Date(item.modifiedAt).toLocaleString())} · ${Math.max(1,Math.round(Number(item.size || 0) / 1024))} KB</small></div><button class="ghost-btn backup-restore-btn" onclick="restoreProjectBackup('${attr(item.name)}')">Restore</button></article>`).join("")}</div>` : `<p class="hint">No rotating backups yet. A backup is created before each validated save.</p>`}<div class="settings-save-line"><span class="settings-save-state" data-state="none" role="status">Nothing on this panel is a setting.</span><small class="settings-save-model">Every control here acts as soon as you use it, so there is nothing to save.</small></div><div class="settings-actions"><a class="ghost-btn" href="#/reports">Open project log & reports</a><button class="ghost-btn" onclick="doExport()">Write saved project as Markdown</button><button class="ghost-btn" onclick="downloadJSON()">Download this window's project JSON</button></div><p class="hint">Markdown is written by the server from the saved project. The JSON download is this window's copy, which can include edits still saving; it is not a restore point.</p><p id="export-note" class="hint" role="status"></p></section>`;
    const panels = { appearance: appearancePanel, files: filesPanel, access: accessPanel, naming: namingPanel, project: projectPanel, assistant: assistantPanel, generation: generationPanel, integrations: integrationsPanel, accounts: accountsPanel, "project-recovery": recoveryPanel };
    setTimeout(() => { if (typeof initSettingsPanel === "function") initSettingsPanel(); }, 0);
    if (selected === "integrations") setTimeout(() => { if (typeof refreshComfyWorkflowList === "function") refreshComfyWorkflowList(); }, 0);
    if (selected === "accounts" || selected === "generation") setTimeout(() => { if (typeof refreshCivitaiGrants === "function") refreshCivitaiGrants(); }, 0);
    return studioSettingsRender({ selected, panels, config: c, health, accountData, panelState });
  },
};
