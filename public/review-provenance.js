...
function candidateReviewModalMarkup(s, frame, take, row, build, guide) {
  ...
  /* THE BRAIDY PANEL OWNS THE BRAIDY REVIEW.
   *
   * The sequence this dialog asks for is: the returned image, then an OPTIONAL
   * Braidy read of it, then the filmmaker's decision. The control that asks
   * Braidy used to sit in the bottom tool group beside BUILD CORRECTION, three
   * hundred pixels from the panel that would show its answer, so nothing on the
   * screen connected the request to the result. It is the same handler, the same
   * capability gate and the same wording about what Braidy is allowed to do.
   *
   * BRAIDY STAYS ADVISORY. This panel states a recommendation and nothing else:
   * approve, keep as alternate and reject are below it, they are the only
   * controls that write a decision, and none of them reads anything from here.
   *
   * RE-ASKING IS NOT NEW BEHAVIOUR. The old control could be pressed with a
   * review already recorded and would replace it; the label now says so instead
   * of leaving the filmmaker to find out.
   *
   * A MARK SLOT, AND NOTHING ABOUT WHO FILLS IT. This file is a production review
   * path, and the assistant's own suite holds a deliberate line across every
   * client file: no production path may know the assistant exists, so that none
   * of them can come to depend on it. Reaching for its art from here would cross
   * that line even though the guard's pattern would not have caught the call --
   * and the guard reads comments too, which is why this one does not name the
   * thing either. So this declares that a mark belongs here and stops. The shell
   * decorates it when it has something to decorate it with; undecorated it is an
   * empty inert span and the panel reads exactly the same. */
  const assistantMark = `<span class="candidate-assistant-mark" data-assistant-mark="1" aria-hidden="true"></span>`;
  const braidyDetail = ai
    ? `<p class="candidate-braidy-summary">${esc(ai.summary || "")}</p><details class="candidate-structured-ai"><summary><b>Braidy notes</b><span>${esc(String(ai.recommendation || "review").toUpperCase())}</span></summary><div class="candidate-structured-ai-body">${Object.entries(ai.categories || {}).map(([key, item]) => `<div><b>${esc(key.replace(/([A-Z])/g, " $1"))} · ${esc(String(item.severity || "pass").toUpperCase())}</b><span>${esc(item.note || "")}</span></div>`).join("")}</div></details>`
    : `<p class="candidate-braidy-invite">Ask Braidy to check the returned image for visible issues.</p>`;
  const braidyPanel = `<section class="candidate-braidy" data-braidy-reviewed="${ai ? "1" : "0"}"><header>${assistantMark}<div><span>Braidy review</span><b>${ai ? esc(String(ai.recommendation || "review").toUpperCase()) : "Not reviewed yet"}</b></div><button class="ghost-btn candidate-braidy-run" onclick="runCandidateStructuredVisionReview('${s.id}','${frame?.id || ""}','${attr(take.name)}')"${aiDisabledAttrs("vision")}>${ai ? "Ask Braidy to review again" : "Ask Braidy to review"}</button></header>${braidyDetail}</section>`;
  ...
  return `<div class="candidate-review-modal ${approved ? "is-approved" : ""}"><header class="candidate-review-title"><div><span>FRAME ${esc(frame?.label || "A")} · CANDIDATE REVIEW</span><h3>${esc(take.name)}</h3><p>Review the image, note any visible issues, then decide what to keep.</p></div><div class="candidate-review-title-actions"><span class="candidate-review-head-status state-${attr(status.tone)}">${esc(status.label)}</span><button class="cancel" onclick="closeModal()">Close</button></div></header>${approved ? `<section class="candidate-review-complete"><i>✓</i><div><b>Frame ${esc(frame?.label || "A")} is approved</b><span>This review is complete. Close the dialog and follow the green Next Action card to continue.</span></div></section>` : ""}<div class="candidate-review-layout"><section class="candidate-review-visual"><div class="candidate-review-view-buttons"><button class="chip on" data-candidate-review-view="candidate" onclick="setCandidateReviewView('candidate',this)">Candidate</button>${guide ? `<button class="chip" data-candidate-review-view="guide" onclick="setCandidateReviewView('guide',this)">Guide</button><button class="chip" data-candidate-review-view="side" onclick="setCandidateReviewView('side',this)">Side by side</button><button class="chip" data-candidate-review-view="overlay" onclick="setCandidateReviewView('overlay',this)">Overlay</button>` : ""}</div><div id="candidate-review-stage" class="candidate-review-stage view-candidate"><figure class="candidate-layer candidate-result"><img src="${attr(take.url)}" alt="Candidate image"><figcaption>Candidate</figcaption></figure>${guide ? `<figure class="candidate-layer candidate-guide"><img src="${attr(guide.url)}" alt="Blocking guide"><figcaption>${esc(guide.name)}</figcaption></figure>` : ""}</div>${guide ? `<label class="candidate-overlay-control"><span>Guide opacity</span><input id="candidate-guide-opacity" type="range" min="0" max="100" value="50" oninput="setCandidateGuideOpacity(this.value)"><output id="candidate-guide-opacity-output">50%</output></label>` : `<div class="candidate-no-guide">No active blocking guide. Composition review uses the written frame requirement.</div>`}${candidateReviewEvidence(s, frame, row, build, guide)}</section><section class="candidate-review-form"><section id="candidate-review-readiness" class="candidate-review-readiness state-${attr(status.tone)}"><i class="candidate-review-state-mark" aria-hidden="true"></i><div><span>REVIEW STATUS</span><b id="candidate-review-readiness-label">${esc(status.label)}</b><small id="candidate-review-readiness-note">${esc(status.note)}</small></div></section>${braidyPanel}<details class="candidate-review-rubric" open><summary><div><span>STRUCTURED RUBRIC</span><b>Open only the categories that need notes</b></div><span>${CANDIDATE_REVIEW_CATEGORIES.length}</span></summary>${categoryRows}</details><details class="candidate-reference-review-section"><summary>Numbered-reference diagnosis <span>${refs.length}</span></summary><div class="candidate-reference-review-body">${referenceRows}</div></details><details class="candidate-review-summary" ${review.summary ? "open" : ""}><summary>Director summary <span>${review.summary ? "Added" : "Optional"}</span></summary><textarea id="candidate-review-summary" placeholder="What works, what fails, and what a correction must preserve.">${esc(review.summary)}</textarea></details></section></div>${candidateCorrectionActivityMarkup(s, frame, row)}<footer class="candidate-review-actions"><div class="candidate-review-tools"><button class="changes-btn" onclick="buildCandidateCorrectionPrompt()">BUILD CORRECTION</button><details class="candidate-review-more"><summary>More</summary><div><button class="ghost-btn" onclick="saveCandidateStructuredReview(false)">SAVE REVIEW</button>${build?.id ? `<button class="chip" onclick="pinCandidateSourceBuild('${s.id}','${attr(take.name)}')">PIN SOURCE BUILD</button><button class="chip" onclick="viewCandidateSourceBuild('${s.id}','${attr(take.name)}')">VIEW SOURCE BUILD</button>` : ""}${(guidedFrameCandidateRows(s, frame, takesFor(s.id), guidedFrames(s).findIndex((item) => item.id === frame?.id)) || []).length > 1 ? `<button class="chip" onclick="chooseCandidateComparison('${s.id}','${frame?.id || ""}','${attr(take.name)}')">COMPARE</button>` : ""}</div></details></div><div class="candidate-review-decisions"><span class="candidate-review-decisions-label">Your decision</span><button class="chip" onclick="setCandidateReviewOutcome('alternate')">KEEP AS ALTERNATE</button><button class="chip danger" onclick="setCandidateReviewOutcome('rejected')">REJECT</button>${approveButton}</div></footer></div>`;
}
...
window.openCandidateCorrectionModal = (shotId, frameId, name, buildId = "") => {
  ...
  /* THE ACCEPTED SLICE 4 PREFLIGHT, not a fourth hand-rolled settings grid.
   *
   * This dialog drew count, quality and resolution side by side whatever the model
   * supported, rendered no plan, and — with a per-image rate configured — showed no price
   * at all before a paid edit request. It renders the same block the blocking-revision and
   * continuity-state dialogs render now, from the same control plan and the same configured
   * image rate, so provider and cost truth is on screen before the paid button and the
   * dispatch is restricted to what the active view actually offered. */
  const correctionIds = {
    count: "candidate-correction-output-count",
    quality: "candidate-correction-quality",
    resolution: "candidate-correction-resolution",
  };
  const drawCorrectionView = (mode) => renderFalFixedImageView("candidate-correction-generation-view", correctionIds,
  ...
  /* The fourth strip that draws a paid generation job, and it had the same omission as
     the entity one: no uncertainty branch, so a submission the server reports as
     uncertain was drawn as ordinary running work with a Cancel the route refuses and no
     way to resolve it. `unknown` is checked first, and the control is the same dialog
     every other strip opens — this file does not get its own recovery flow either. */
  const unknown = job ? job.uncertain === true : false;
  const jobMarkup = job ? `<div class="fal-job-strip ${unknown ? "unresolved" : active ? "active" : job.status === "COMPLETED" ? "done" : job.status === "FAILED" ? "failed" : ""}"><div><span>${unknown ? "?" : active ? '<i class="spin">◌</i>' : job.status === "COMPLETED" ? "✓" : job.status === "FAILED" ? "!" : "·"}</span><div><b>${esc(typeof falJobStatusLabel === "function" ? falJobStatusLabel(job) : job.status)}</b><small>${unknown && typeof falUnresolvedExplanation === "function" ? esc(falUnresolvedExplanation(job)) : `${esc(job.model || "GPT Image 2 Edit")}${job.error ? ` · ${esc(job.error)}` : ""}`}</small></div></div><div>${unknown ? `<button class="chip" onclick="openFalUnresolvedModal('${attr(job.id)}')">Check and resolve</button>` : active ? `<button class="chip" onclick="refreshCandidateCorrectionGeneration('${job.id}')">Refresh</button><button class="chip danger" onclick="cancelCandidateCorrectionGeneration('${job.id}')">Cancel</button>` : job.status === "FAILED" ? `<span>Adjust the correction and try again.</span>` : ""}</div></div>` : "";
