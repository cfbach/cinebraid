...
window.updateSceneAutomationEstimate = () => {
  ...
  if (estimateEl) estimateEl.innerHTML = `<b>${generationIds.length} generation target${generationIds.length === 1 ? "" : "s"} · ${approvedContext} approved context shot${approvedContext === 1 ? "" : "s"} · cap ${cap}</b><span>Mode: ${modeLabel}. Worst-case paid image plan: ${estimate.total}. Reused approved shots do not consume generation passes.</span>`;
  /* The scene planner prices its cap from the same configured rate as every other
     generation surface, so a scene run and the shot runs inside it cannot quote
     differently for the same images. */
  if (typeof v6211DrawGenerationView === "function") v6211DrawGenerationView("scene", {
    maxImages: cap,
    limits: {
      rows: [
        { value: generationIds.length, label: generationIds.length === 1 ? "shot to generate" : "shots to generate" },
        { value: cap, label: "images at the very most" },
      ],
      stopEarly: "Shots already approved are reused rather than generated again, and the run stops at this confirmed cap.",
    },
  });
  const button = document.getElementById("v640-start-scene");
