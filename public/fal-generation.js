...
window.openFalEntityGenerationModal = (list, entityId, buildId = "", stateId = "", options = {}) => {
  ...
  /* REFERENCE_FIRST_CANON_SIMPLIFICATION_V1 — THE PROMPT THIS REQUEST CARRIES, SHOWN BEFORE
     ANYTHING IS SENT. "Generate primary reference" reaches this dialog without passing the
     manual panel where a prepared prompt used to be read, so the dialog now shows it itself.
     It is composed by entityGenerationPrompt(), the same function the submission calls,
     over the same build and the same references. It is read-only, because a changed prompt
     is a new build and belongs to the manual path under More options. */
  const sentPrompt = entityGenerationPrompt(list, entity, build, refs, { state, mode: effectiveMode });
  /* The shipped rule for every dialog that asks for money: clear the notice already
     showing. "Reference prompt compiled" otherwise sits over this confirmation, and at
     phone width it covers the controls. */
  dismissStaleNotice();
  const promptMarkup = `<details class="fal-entity-prompt" data-fal-entity-prompt><summary>Prompt that will be sent <span>${esc(`${sentPrompt.length.toLocaleString()} characters · ${build.llmUsed ? "assistant-shaped" : "rules-compiled"}`)}</span></summary><pre>${esc(sentPrompt)}</pre></details>`;
  openModal(`<h3>Generate ${state ? `${esc(state.name || "state")} ` : ""}${esc(typeLabel)} reference candidates</h3><div class="modal-sub">FAL · GPT IMAGE 2${refs.length ? " EDIT / REFERENCE-GUIDED" : " TEXT-TO-IMAGE"}</div>${stateSummary}<div class="candidate-evidence-facts"><span>${esc(entity.id)}</span>${state ? `<span>Target · ${esc(state.name || "State")}</span>` : ""}<span>${refs.length} input${refs.length === 1 ? "" : "s"}</span><span>Candidate only · approval required</span></div><div class="fal-entity-format-facts" data-fal-entity-aspect="${attr(aspectLabel)}"><div><span>Aspect ratio</span><b>${esc(aspectLabel)}</b><small>Fixed by the ${esc(typeLabel)} reference format. The prompt below was compiled at this ratio, so it is stated here rather than offered as a control that could disagree with it.</small></div></div><div id="fal-entity-generation-view"></div>${promptMarkup}<p class="hint">This submits a paid FAL image request. Returned files are added as unapproved ${esc(typeLabel)} candidates${state ? ` targeted to ${esc(state.name || "this state")}` : ""}. They do not become canon until you explicitly approve one.</p><div class="modal-actions fal-entity-actions"><button class="cancel" onclick="closeModal()">Cancel</button><button class="approve-btn large" onclick="startFalEntityGeneration()">START GENERATION</button></div>`,
    typeof options.resolveReturnFocus === "function" ? { resolveReturnFocus: options.resolveReturnFocus } : {});
