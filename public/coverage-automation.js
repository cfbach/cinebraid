...
(function () {
  ...
  function coverageCropPanelOptions(layout, selected = 0) {
    const preset = COVERAGE_CROP_LAYOUTS[layout] || COVERAGE_CROP_LAYOUTS["3x1"];
    /* EV2-7 — NOTHING HERE DETECTS PANELS. These are evenly spaced SUGGESTED CROPS over an
       assumed grid, and they are named for what they are. The state keys (panelIndex, layout)
       and the provenance they are stored under are unchanged. */
    return Array.from({ length: preset.cols * preset.rows }, (_, index) => `<option value="${index}" ${index === selected ? "selected" : ""}>Suggested crop ${index + 1}</option>`).join("");
  }
  ...
  function coverageCropFineTune(layout, panelIndex, fileName) {
    return `<details class="coverage-crop-advanced"><summary>Fine tune</summary><div><div class="coverage-crop-quick-grid"><label><span>Sheet layout</span><select id="coverage-crop-layout" onchange="setCoverageCropLayout(this.value)">${Object.entries(COVERAGE_CROP_LAYOUTS).map(([id,preset]) => `<option value="${id}" ${id === layout ? "selected" : ""}>${preset.label}</option>`).join("")}</select></label><label><span>Suggested crop</span><select id="coverage-crop-panel" onchange="setCoverageCropPanel(this.value)">${coverageCropPanelOptions(layout,panelIndex)}</select></label></div><div class="coverage-crop-presets"><button onclick="applyCoverageCropPreset()">Reset to suggested crop</button><button onclick="setCoverageCropFull()">Use full image</button></div><div class="coverage-crop-fields">${["x","y","w","h"].map((key) => `<label><span>${key.toUpperCase()} %</span><input id="coverage-crop-${key}" type="number" min="0" max="100" step="0.5" onchange="setCoverageCropField('${key}',this.value)"></label>`).join("")}</div><label><span>Extraction note</span><textarea id="coverage-crop-note" placeholder="Why this crop is authoritative or any limitations."></textarea></label><div class="coverage-crop-provenance"><b>Source sheet</b><span>${esc(fileName)}</span><small>CineBraid stores the crop coordinates, the sheet grid it assumed, the suggested-crop position and the source file so the view can be traced later.</small></div></div></details>`;
  }
  ...
  function openGuidedCoverageExtractor(list, entity, fileName, media, row, guided) {
    ...
    const disclosure = guided.source?.kind === "media" ? "Production Media sheet · the original is not changed and is not added to this reference." : recorded ? "" : "No state recorded on this sheet (it does not change " + stateName + "). The crop is recorded for " + stateName + ".";
  ...
  window.openCoverageSheetExtractor = (list, entityId, fileName, allowHumanOverride = false, guided = null) => {
     ...
     * does. */
    openModal(`<div class="coverage-extractor-modal"><header><div><span>REFERENCE EXTRACTION</span><h3>${esc(entity.name || entity.id)}</h3><p>Turn one sheet into clean authority views. Select a target, choose a suggested crop, fine-tune only when needed, then save.</p></div><button class="cancel" onclick="closeModal()">Close</button></header><nav class="coverage-extractor-steps" aria-label="Reference extraction steps"><span class="active"><b>1</b> Choose target</span><span class="active"><b>2</b> Crop one view</span><span><b>3</b> Save authority</span></nav><div class="coverage-extractor-layout"><div><div id="coverage-crop-stage" class="coverage-crop-stage"><img id="coverage-crop-source" src="${attr(media.url)}" alt="Reference sheet"><div id="coverage-crop-overlay" class="coverage-crop-overlay"><span id="coverage-crop-target-label"></span></div></div><small>Drag anywhere on the sheet to redraw the crop. The layout and suggested-crop controls provide a fast starting point. CineBraid does not detect panels.</small></div><aside><label><span>Save this crop as</span><select id="coverage-crop-slot" onchange="selectCoverageCropSlot(this.value)">${slots.map((slot) => `<option value="${attr(slot.id)}" ${slot.id === window._coverageCrop.slotId ? "selected" : ""}>${esc(slot.label)}${slotSelectedFile(slot) ? " · already assigned" : ""}</option>`).join("")}</select></label><div class="coverage-crop-navigation"><button class="ghost-btn" onclick="stepCoverageCropPanel(-1)">← Previous suggested crop</button><button class="ghost-btn" onclick="stepCoverageCropPanel(1)">Next suggested crop →</button></div><div class="coverage-review-gate"><b>Review is optional</b><span>Save crop &amp; use puts this crop into the view above in one action. Save as candidate keeps it without changing the view. Neither is a canon approval, and an AI check can be run afterwards either way.</span></div>${coverageCropFineTune(layout, panelIndex, fileName)}</aside></div><div class="modal-actions coverage-extractor-actions"><button class="cancel" onclick="closeModal()">Cancel</button><button class="ghost-btn" onclick="extractCoverageCrop({ assign: false })">SAVE AS CANDIDATE</button>${moreToCrop ? `<button class="ghost-btn" onclick="extractCoverageCrop({ assign: false, next: true })">SAVE & NEXT VIEW</button>` : ""}${nondefaultSource ? `<p class="coverage-crop-state-note" role="note">Assign ${esc(stateNameOf(entity, legacyStateId))} views from the Reference Desk → Build coverage.</p>` : `<button class="approve-btn large" onclick="extractCoverageCrop({ assign: true })">SAVE CROP & USE</button>`}</div></div>`);
    setTimeout(() => { coverageCropRender(); coverageCropBindStage(); }, 30);
  };
  ...
  window.seedCoverageFromPrimary = (list, entityId) => {
    ...
    if (typeof entityCandidateIsCoverageSheet === "function" && entityCandidateIsCoverageSheet(entity, primary.name)) return toast("This is a multi-view sheet. Crop one view at a time instead of assigning the whole sheet to one angle.");
    const slots = typeof ensureCoverageSlots === "function" ? ensureCoverageSlots(list, entity) : entity.coverageSlots || [];
    if (list === "characters") {
