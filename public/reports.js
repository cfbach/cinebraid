...
function reportsDetailMarkup(report) {
  ...
  /* "Human approvals" must contain human approvals. It used to admit any
     completed step whose kind contained "approval", which is how an
     automation-written frame-approval step appeared under that heading in the
     run report. `humanApproved` is the mark a person's act writes and the only
     thing this section may count. */
  const run = report.run || {}, analysis = report.analysis || {}, approvals = Object.values(run.steps || {}).filter((step) => step.result?.humanApproved === true);
  const errors = Object.values(run.steps || {}).filter((step) => step.error || step.status === "failed");
  /* PASS PROGRESSION, MOUNTED WHERE 28ee913 SAID IT WENT.
     A1 retired v626RunReportMarkup from the creative surface and named pass
     progression among the six things that "genuinely belong to Reports" — but
     nothing here ever picked it up, so v666ReferenceProgressionMarkup kept
     rendering correctly for its Node test and was reachable from no screen at
     all. This is that move completed, not a second report: the same renderer,
     the same run, read through the run this view already loaded.
     It renders only for a run that really has passes — the renderer returns ""
     otherwise — so a shot-chain run gains no empty section. */
  const passes = Array.isArray(run.result?.referencePasses) ? run.result.referencePasses : [];
  const progression = typeof v666ReferenceProgressionMarkup === "function" ? v666ReferenceProgressionMarkup(run) : "";
  const progressionSection = progression
    ? `<details class="reports-detail-section reports-pass-progression" open><summary>Pass progression <span>${passes.length}</span></summary>${progression}</details>`
    : "";
  return `<section class="reports-detail"><header><div><span>RUN DETAIL</span><h2>${esc(run.label || run.targetId || run.id)}</h2><p>${esc(run.summary || run.stage || "No summary recorded.")}</p></div><div class="reports-detail-status state-${attr(reportsStatusTone(run.status))}"><b>${esc(reportsStatusLabel(run.status))}</b><small>${esc(run.id || "")}</small></div></header><div class="reports-detail-actions"><button class="ghost-btn" onclick="copyAutomationSupportSummary('${attr(run.id)}')">COPY SUPPORT SUMMARY</button><button class="ghost-btn" onclick="copyAutomationDebugReport('${attr(run.id)}')">COPY DEBUG JSON</button><button class="ghost-btn" onclick="downloadAutomationRunReport('${attr(run.id)}')">DOWNLOAD RAW JSON</button><button class="approve-btn" onclick="downloadAutomationDiagnosticBundle('${attr(run.id)}')">DOWNLOAD DIAGNOSTIC BUNDLE</button>${run.status === "completed" ? `<button class="changes-btn" onclick="flagAutomationRunInefficient('${attr(run.id)}')">FLAG INEFFICIENT</button>` : ""}<a class="ghost-btn" href="${attr(typeof v641RunRoute === "function" ? v641RunRoute(run) : "#/production")}">OPEN WORKSPACE</a></div><div class="reports-summary-grid"><article><span>Revision</span><b>${Number(run.revision || 0)}</b></article><article><span>Images generated</span><b>${Number(run.usage?.imagesGenerated || 0)}</b>${Number(run.config?.maxImages) > 0 ? `<small>of ${Number(run.config.maxImages)} confirmed</small>` : ""}</article><article><span>Recorded requests</span><b>${Number(run.usage?.imageRequests || 0)}</b></article><article><span>Accepted requests</span><b>${Number(analysis.providerRequestsAccepted || 0)}</b></article><article><span>Review calls</span><b>${Number(run.usage?.reviewCalls || 0)}</b></article><article><span>Highest pass</span><b>${Number(analysis.highestPassUsed || 0)}</b></article><article><span>Elapsed</span><b>${esc(reportsDuration(run))}</b></article><article><span>Classification</span><b>${esc(String(analysis.classification || "unknown").replace(/_/g, " "))}</b></article>${reportsRecordedCostMarkup(report.recordedCost)}</div>${analysis.warnings?.length ? `<section class="reports-analysis-warnings"><header><b>Optimization signals</b><span>${analysis.warnings.length}</span></header>${analysis.warnings.map((item) => `<p>${esc(item)}</p>`).join("")}</section>` : ""}<details class="reports-detail-section" open><summary>Step timeline <span>${Object.keys(run.steps || {}).length}</span></summary>${reportsTimelineMarkup(run)}</details>${progressionSection}<details class="reports-detail-section"><summary>Prompts and revisions <span>${(report.prompts || []).length}</span></summary>${reportsPromptsMarkup(report.prompts)}</details><details class="reports-detail-section"><summary>Review records <span>${(report.reviews || []).length}</span></summary>${reportsReviewsMarkup(report.reviews)}</details><details class="reports-detail-section"><summary>Reference manifest <span>${(report.referenceManifest || []).length}</span></summary>${reportsReferenceMarkup(report.referenceManifest)}</details><details class="reports-detail-section"><summary>Provider jobs and operation keys <span>${(report.providerJobs || []).length}</span></summary>${reportsProviderJobsMarkup(report.providerJobs)}</details><details class="reports-detail-section"><summary>Human approvals <span>${approvals.length}</span></summary><pre>${esc(JSON.stringify(approvals, null, 2))}</pre></details><details class="reports-detail-section"><summary>Logs and errors <span>${(run.logs || []).length + errors.length}</span></summary><div class="reports-log-list">${(run.logs || []).map((entry) => `<p class="${attr(entry.tone || "info")}"><b>${esc(reportsTime(entry.at))}</b>${esc(entry.message || "")}</p>`).join("") || `<div class="reports-empty"><span>No run logs recorded.</span></div>`}</div>${errors.length ? `<pre>${esc(JSON.stringify(errors, null, 2))}</pre>` : ""}</details><details class="reports-detail-section reports-efficiency-feedback"><summary>Efficiency feedback <span>${run.feedback?.inefficient ? "FLAGGED" : "OPTIONAL"}</span></summary><div><textarea id="automation-efficiency-note" placeholder="Describe repeated complaints, unnecessary passes, weak first-round prompting, wrong references, or other avoidable work.">${esc(run.feedback?.note || "")}</textarea><button class="changes-btn" onclick="saveAutomationEfficiencyFeedback('${attr(run.id)}')">SAVE EFFICIENCY FEEDBACK</button></div></details>${run.feedback?.inefficient ? `<section class="reports-user-feedback"><span>USER EFFICIENCY NOTE</span><p>${esc(run.feedback.note || "Flagged without a note")}</p></section>` : ""}</section>`;
}
/* WHAT IT COST IS READ HERE, NEVER RECOMPUTED HERE.
 *
 * This used to be `imagesGenerated × CONFIG.generation.fal.estimatedCostPerImage` —
 * today's Settings number applied backwards over all of history. Editing that field
 * rewrote what every past run appeared to have cost, a 512px draft and a 4K render
 * priced identically, and a project whose owner never filled the field in reported a
 * confident $0.00. The figure below is instead summed from the estimates the jobs
 * themselves recorded at submission, and the server does the summing.
 *
 * The three populations stay visible rather than collapsing into one dollar figure,
 * because a partial record that looks complete is worse than one that admits the gap:
 * jobs that predate cost recording say so, and jobs submitted with no configured rate
 * say that instead of passing as free.
 *
 * Everything here is an ESTIMATE. CineBraid gets no billing figure back from fal, so
 * nothing on this screen may be worded as confirmed spend. */
function reportsRecordedCostMarkup(cost) {
  const jobs = Number(cost?.jobs || 0);
  if (!cost || !jobs) return "";
  const priced = Number(cost.priced || 0), unpriced = Number(cost.unpriced || 0), unrecorded = Number(cost.unrecorded || 0);
  const gaps = [
...
function reportsOptimizationMarkup(summary) {
  if (!summary) return "";
  return `<details class="reports-optimization"><summary><div><span>WHERE EFFORT WENT</span><b>Totals from ${plural(Number(summary.runCount || 0), "recorded run")}</b></div><span>EXPAND</span></summary><div class="reports-optimization-body"><div class="reports-summary-grid"><article><span>Total generated images</span><b>${Number(summary.totals?.imagesGenerated || 0)}</b></article><article><span>Provider requests accepted</span><b>${Number(summary.totals?.providerRequestsAccepted || 0)}</b></article><article><span>Assistant calls</span><b>${Number(summary.totals?.assistantCalls || 0)}</b></article><article><span>Review calls</span><b>${Number(summary.totals?.reviewCalls || 0)}</b></article><article><span>First-pass completion rate</span><b>${Number(summary.quality?.firstPassSuccessRate || 0)}%</b></article><article><span>Failed before acceptance</span><b>${Number(summary.quality?.failedBeforeAcceptance || 0)}</b></article><article><span>Failed after acceptance</span><b>${Number(summary.quality?.failedAfterAcceptance || 0)}</b></article><article><span>Human approvals</span><b>${Number(summary.quality?.humanApprovals || 0)}</b></article>${reportsRecordedCostMarkup(summary.totals?.recordedCost)}</div><div class="reports-optimization-columns"><section><header><b>Highest-effort targets</b></header>${(summary.highestEffortTargets || []).map((row) => `<article><div><b>${esc(row.label || row.targetId)}</b><small>${esc(row.targetId)} · ${Number(row.runs || 0)} runs</small></div><span>pass ${Number(row.highestPassUsed || 0)} · ${Number(row.images || 0)} images</span></article>`).join("") || `<div class="reports-empty"><span>No generation history yet.</span></div>`}</section><section><header><b>Repeated review complaints</b></header>${(summary.repeatedComplaints || []).map((row) => `<article><p>${esc(row.reason)}</p><span>${Number(row.count || 0)} repeats</span></article>`).join("") || `<div class="reports-empty"><span>No repeated complaint clusters detected.</span></div>`}</section><section><header><b>User inefficiency notes</b></header>${(summary.inefficientFeedback || []).map((row) => `<article><div><b>${esc(row.label || row.targetId)}</b><p>${esc(row.note)}</p></div><a href="#/reports/${encodeURIComponent(row.runId)}">VIEW RUN</a></article>`).join("") || `<div class="reports-empty"><span>No runs have been flagged inefficient.</span></div>`}</section></div></div></details>`;
}
