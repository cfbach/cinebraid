...
function reportsOptimizationMarkup(summary) {
  ...
  return `<details class="reports-optimization"><summary><div><span>PROJECT OPTIMIZATION SUMMARY</span><b>Production intelligence from ${Number(summary.runCount || 0)} durable runs</b></div><span>EXPAND</span></summary><div class="reports-optimization-body"><div class="reports-summary-grid"><article><span>Total generated images</span><b>${Number(summary.totals?.imagesGenerated || 0)}</b></article><article><span>Provider requests accepted</span><b>${Number(summary.totals?.providerRequestsAccepted || 0)}</b></article><article><span>Assistant calls</span><b>${Number(summary.totals?.assistantCalls || 0)}</b></article><article><span>Review calls</span><b>${Number(summary.totals?.reviewCalls || 0)}</b></article><article><span>First-pass completion rate</span><b>${Number(summary.quality?.firstPassSuccessRate || 0)}%</b></article><article><span>Failed before acceptance</span><b>${Number(summary.quality?.failedBeforeAcceptance || 0)}</b></article><article><span>Failed after acceptance</span><b>${Number(summary.quality?.failedAfterAcceptance || 0)}</b></article><article><span>Human approvals</span><b>${Number(summary.quality?.humanApprovals || 0)}</b></article>${rate ? `<article><span>Configured cost estimate</span><b>$${estimated.toFixed(2)}</b><small>At $${rate.toFixed(3)} per image; not provider billing.</small></article>` : ""}</div><div class="reports-optimization-columns"><section><header><b>Highest-effort targets</b></header>${(summary.highestEffortTargets || []).map((row) => `<article><div><b>${esc(row.label || row.targetId)}</b><small>${esc(row.targetId)} · ${Number(row.runs || 0)} runs</small></div><span>pass ${Number(row.highestPassUsed || 0)} · ${Number(row.images || 0)} images</span></article>`).join("") || `<div class="reports-empty"><span>No generation history yet.</span></div>`}</section><section><header><b>Repeated review complaints</b></header>${(summary.repeatedComplaints || []).map((row) => `<article><p>${esc(row.reason)}</p><span>${Number(row.count || 0)} repeats</span></article>`).join("") || `<div class="reports-empty"><span>No repeated complaint clusters detected.</span></div>`}</section><section><header><b>User inefficiency notes</b></header>${(summary.inefficientFeedback || []).map((row) => `<article><div><b>${esc(row.label || row.targetId)}</b><p>${esc(row.note)}</p></div><a href="#/reports/${encodeURIComponent(row.runId)}">VIEW RUN</a></article>`).join("") || `<div class="reports-empty"><span>No runs have been flagged inefficient.</span></div>`}</section></div></div></details>`;
}
...
function reportsLegacyCompatibilityUsage() {
  ...
  const rows = [
    { label: "Legacy shot status fallback", count: shots.filter((shot) => shot.status && !shot.workflowStatus).length, detail: "Still read for older projects; current lifecycle uses workflowStatus." },
    { label: "Legacy VO note fallback", count: shots.filter((shot) => shot.audio?.vo && !shot.audio?.line).length, detail: "Still read when exact spoken line has not been migrated." },
    { label: "Legacy continuity-delta fields", count: entities.reduce((total, entity) => total + (entity.continuityStates || []).filter((state) => !state.notes && [state.description, state.stateDelta, state.changeOnly, state.instructions].some((value) => String(value || "").trim())).length, 0), detail: "Migrated into canonical state notes when the project normalizer runs." },
    { label: "Legacy single-winner stills", count: shots.filter((shot) => shot.winner && !(shot.keyframes || []).some((frame) => frame.winner)).length, detail: "Preserved so older approved shots remain visible while frame records are normalized." },
  ];
...
function reportsProjectLogMarkup(docs = []) {
  ...
  return `<details class="reports-project-log"><summary><div><span>PROJECT LOG</span><b>${issues.length} issue${issues.length===1?"":"s"} · ${docs.length} source document${docs.length===1?"":"s"} · ${legacy.reduce((n,row)=>n+row.count,0)} legacy use${legacy.reduce((n,row)=>n+row.count,0)===1?"":"s"}</b></div><span>EXPAND</span></summary><div class="project-log-grid"><section><h4>Integrity checks</h4>${issues.length?`<div class="health-issue-list">${issues.slice(0,20).map((x)=>`<a href="#/shot/${x.shot.id}"><b>${esc(x.shot.id)}</b><small>${esc(x.msg)}</small></a>`).join("")}</div>`:`<p class="hint">No structural issues found.</p>`}</section><section><h4>Recent record</h4>${recent.length?`<div class="project-log-list">${recent.map((x)=>`<article><b>${esc(x.date || "")}</b><span>${esc(x.text || x.summary || x.decision || "")}</span></article>`).join("")}</div>`:`<p class="hint">No recent decisions or sessions.</p>`}</section><section class="project-legacy-usage"><h4>Compatibility telemetry</h4>${legacy.length?`<div class="project-log-list">${legacy.map((row)=>`<article><b>${row.count} · ${esc(row.label)}</b><span>${esc(row.detail)}</span></article>`).join("")}</div>`:`<p class="hint">No tracked legacy project fields are currently in use.</p>`}<small>Telemetry is read-only. Compatibility adapters are removed only after representative old projects report zero use.</small></section></div></details>`;
}
...
async function reportsView(selectedId = "") {
  ...
  try {
    ...
    return `<div class="view-head reports-head"><div><div class="eyebrow">Reports</div><span class="view-title">Runs & production intelligence</span><div class="view-sub">Inspect durable automation history, download support-safe diagnostics, and identify repeated generation waste without crowding the work surface.</div></div><div class="view-head-actions"><button class="approve-btn" onclick="openProductionSummaryExport()">Export production summary</button></div></div>${reportsProjectLogMarkup(docs)}${reportsOptimizationMarkup(summaryData.summary)}<div class="reports-layout">${reportsHistoryMarkup(runs, selectedId, historyData, historyData.targetOptions || [])}${reportsDetailMarkup(report)}</div>`;
  } catch (error) {
