...
function continuityStateCandidateTray(list, entity, state, media) {
  ...
  const source = state.isDefault ? "Primary authority" : parent?.fileName ? `Derived from approved ${parent.label}` : "Generated independently — approved parent unavailable";
...
function continuityStateValidationMarkup(list, entity, state, media = []) {
  ...
  const thumbs = targetMedia && parentMedia ? `<div class="state-validation-thumbs"><button type="button" onclick="openMediaTheatre('${attr(encodeURIComponent(parentMedia.url))}','${attr(encodeURIComponent(`${parentInfo.label} parent authority · ${parentInfo.fileName}`))}','image')"><img src="${attr(parentMedia.url)}" alt="Parent authority"><span>${esc(parentInfo.label)} · parent</span></button><i>→</i><button type="button" onclick="openMediaTheatre('${attr(encodeURIComponent(targetMedia.url))}','${attr(encodeURIComponent(`${state.name || "State"} approved authority · ${state.approvedFile}`))}','image')"><img src="${attr(targetMedia.url)}" alt="Target state authority"><span>${esc(state.name || "State")} · target</span></button></div>` : "";
...
function continuityStatesPanel(list, it, media = []) {
  ...
  const rail = `<nav class="continuity-state-rail" aria-label="Continuity states">${states.map((state, index) => {
    ...
    const status = state.isDefault ? "Default authority" : approved ? "Approved" : referenceRequirementLabel(state);
...
window.setCoverageSlotField = (list, id, index, key, value) => {
  ...
  if (previous && next && previous !== next) {
    return confirmModal(`Replace ${slot.label}?`, () => applyCoverageAssignment(list, entity, slot, next, "manual-replacement"), { title: "Replace approved coverage authority", confirmLabel: "REPLACE VIEW", body: `${previous} will remain in history, but ${next} becomes the new approved authority.` });
  }
...
window.approveCoverageCandidate = (list, id, fileName, slotId, directOverride = false) => {
  ...
  const replaceOrCommit = () => {
    if (slot.approvedFile && slot.approvedFile !== fileName) return confirmModal(`Replace ${slot.label}? ${slot.approvedFile} will remain in replacement history.`, commit, { title: "Replace approved reference authority", confirmLabel: "REPLACE VIEW" });
    commit();
  };
...
function boundedEntityTaskStatus(list, entity, taskId, activeCandidates, states) {
  ...
  if (taskId === "reference") return primary ? {tone:"complete",label:"Approved"} : {tone:"attention",label:"Needed"};
  if (taskId === "review") return activeCandidates.length ? {tone:"attention",label:`${activeCandidates.length} to choose`} : {tone:"optional",label:"Clear"};
  if (taskId === "coverage") {
    ...
    return active ? {tone:"active",label:"Generating"} : missing ? {tone:"attention",label:`${missing} missing`} : planned ? {tone:"pending",label:`${planned} planned`} : {tone:"complete",label:"Complete"};
  }
  if (taskId === "details") {
    const hasDetails = String(entity.block || entity.notes || entity.creationDescription || "").trim() || (entity.planningMedia || []).length;
    return hasDetails ? {tone:"complete",label:"Recorded"} : {tone:"pending",label:"Add details"};
  }
  if (taskId === "history") {
    const count=(entity.made || []).length + (entity.prompts || []).length;
    return count ? {tone:"complete",label:`${count} records`} : {tone:"optional",label:"None"};
  }
...
function boundedEntityTaskbarMarkup(list, entity, specs, selectedId, activeCandidates, states) {
  ...
  return `<nav class="focused-taskbar bounded-entity-taskbar clarity-taskbar" aria-label="Reference workspaces">${specs.map((spec)=>{const status=boundedEntityTaskStatus(list,entity,spec.id,activeCandidates,states);return `<button type="button" class="focused-task-button tone-${status.tone} ${spec.id===selectedId?"selected":""}" onclick="selectBoundedTask('entity-task','${attr(context)}','${attr(spec.id)}')"><i></i><span><b>${esc(spec.label)}</b><small>${esc(spec.detail || "")}</small></span><em>${esc(status.label)}</em></button>`;}).join("")}</nav>`;
}
function entityAuthoritySummaryMarkup(list, entity, states, mediaByName, selectedTask = "") {
  ...
  const authorityStatus = `<b class="entity-authority-status"><strong>${approvedCount}/${states.length}</strong><span>states assigned</span></b>`;
  if (selectedTask === "coverage") return `<section class="entity-authority-summary is-compact"><header><div><span class="entity-authority-label">APPROVED AUTHORITY</span>${authorityStatus}<small>State authorities are managed in the focused editor below.</small></div></header></section>`;
